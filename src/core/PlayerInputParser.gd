...
static func sanitize_input(input_text: String) -> String:
	...
	if final_result != input_text:
		print(
			(
				"[PromptSanitizer] Sanitized input. Original: '%s' | Sanitized: '%s'"
				% [input_text, final_result]
			)
		)
...
static func parse_input(input_text: String) -> Dictionary:
	...
	if trimmed.is_empty():
		return {"dialogue": "", "action": "", "detected_format": "none"}
	...
	if _is_pure_action_heuristic(trimmed):
		return {"dialogue": "", "action": trimmed, "detected_format": "pure_action"}

	# Default/Undifferentiated: No punctuation styling, could be either bare dialogue or mixed
	return {"dialogue": trimmed, "action": "", "detected_format": "none"}
...
static func _parse_quotes(text: String) -> Dictionary:
	...
	return {"dialogue": dialogue, "action": action, "detected_format": "quotes"}
...
static func _parse_asterisks(text: String) -> Dictionary:
	...
	return {"dialogue": dialogue, "action": action, "detected_format": "asterisks"}
