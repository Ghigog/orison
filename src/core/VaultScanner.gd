...
static func scan_vault(vault_path: String) -> Dictionary:
	...
	for folder in folder_files.keys():
		...
		var votes = {"character": 0, "location": 0, "scene": 0, "lore": 0}
		...
		print(
			(
				"[VaultScanner] Folder: '%s' | Segments: %s | LastSegments: %s | Heuristic: '%s'"
				% [folder, str(segments), str(last_segments), heuristic_type]
			)
		)
	...
	for folder in folder_files.keys():
		...
		for file_path in files:
			...
			var entry = {"id": node_id, "label": label, "file_path": file_path, "body": body}
			...
			if type == "scene":
				if not body.strip_edges().is_empty():
					results.potential_scenes.append(
						{"id": node_id, "label": label, "file_path": file_path}
					)
			elif type == "character":
...
static func _scan_dir_recursive(
	dir_path: String,
