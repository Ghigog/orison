...
func _run_summarization(
	char_id: String, logs_to_summarize: Array, is_session_summary: bool
) -> void:
	...
	prompt += (
		"You are the memory summarization assistant. Summarize the following dialogue segment between the Player and %s in a single, compact, objective paragraph (maximum 3-4 sentences).\n"
		% char_name
	)
	...
	print(
		(
			"[MemoryManager] Requesting summary for character: %s (%d entries, is_session: %s)..."
			% [char_id, logs_to_summarize.size(), str(is_session_summary)]
		)
	)
...
func _apply_medium_term_summary(
	char_id: String, summary: String, logs_to_summarize: Array, is_session_summary: bool
) -> void:
	var character = CampaignState.get_character(char_id)
	...
	for log in current_logs:
		...
		for s_log in logs_to_summarize:
			if (
				log.get("timestamp") == s_log.get("timestamp")
				and log.get("content") == s_log.get("content")
				and log.get("sender") == s_log.get("sender")
			):
				should_remove = true
				break
...
func distill_long_term_memory(char_id: String) -> void:
	...
	prompt += (
		"You are the memory coordinator. Distill the following chronological list of event summaries involving the character %s and the Player into a single, cohesive, long-term memory narrative block (1-2 paragraphs).\n"
		% char_name
	)
	...
	print(
		(
			"[MemoryManager] Requesting long-term memory distillation for character: %s (%d summaries)..."
			% [char_id, mt_memories.size()]
		)
	)
...
func _apply_long_term_distillation(
	char_id: String, distilled_ltm: String, distilled_summaries: Array
) -> void:
	var character = CampaignState.get_character(char_id)
	if character.is_empty():
		return
