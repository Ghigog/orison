...
func _add_node_internal(
	...
	nodes[node_id] = {"label": label, "type": type, "desc": description, "properties": properties}
	graph["nodes"] = nodes
...
func remove_node(node_id: String) -> void:
	if not _has_custom_graph and CampaignState and CampaignState.has_method("apply_state_change"):
		CampaignState.apply_state_change({"type": "remove_graph_node", "node_id": node_id})
	else:
...
func _add_edge_internal(
	from_node: String, to_node: String, relation: String, weight: float = 1.0
) -> void:
	var graph = _get_graph()
	var edges: Array = graph.get("edges", [])
...
func retrieve_context(user_prompt: String, max_tokens: int = -1, target_level: int = 0) -> String:
	...
	keyword_matches.sort_custom(func(a, b): return lexical_scores[a] > lexical_scores[b])
	...
	if not query_vector.is_empty() and EmbeddingStore.has_embeddings():
		...
		if query_vector.is_empty() and not LLMClient.mock_response_handler.is_valid():
			print(
				"[KnowledgeGraph] Warning: nomic-embed-text not available. Falling back to keyword search only."
			)
	...
	merged_ids.sort_custom(func(a, b): return rrf_scores[a] > rrf_scores[b])
	...
	for edge in edges:
		...
		if context_nodes.has(f) and context_nodes.has(t):
			...
			connection_texts.append(
				(
					"  - Relation: %s is [%s] -> %s"
					% [from_lbl, edge.get("relation", "connected"), to_lbl]
				)
			)
	...
	if max_tokens > 0 and estimated_tokens > max_tokens:
		print(
			(
				"[KnowledgeGraph] Lore context (%d tokens) exceeds budget (%d tokens). Truncating distant lore connections."
		...
		for id in merged_ids:
			...
			if prospective_tokens <= max_tokens:
				...
				var label_format = (
					"- **%s** (%s): " % [node.get("label", id), node.get("type", "Entity")]
				)
...
func get_entities_connected_to(
	entity_id: String, edge_type: String, max_depth: int = 1
) -> Array[Dictionary]:
...
func _traverse_graph(
	current_id: String, edge_type: String, current_depth: int, max_depth: int, visited: Dictionary
) -> void:
...
## grouping mechanism.
static func _lexical_score(
	query_terms: PackedStringArray, node: Dictionary, node_id: String
) -> float:
