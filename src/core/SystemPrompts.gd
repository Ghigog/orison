...
static func get_character_agent_prompt(
	...
	prompt += (
		"- Relationship with Player: %s (Affinity Score: %.2f on a scale of -1.0 Nemesis to +1.0 Best Friend)\n\n"
		% [CharacterProfile.get_relationship_label(affinity), affinity]
	)

	prompt += "EMOTIONAL PROFILE:\n"
	prompt += (
		"- Active Emotion: %s (Intensity: %.1f/1.0)\n" % [active_emotion.capitalize(), intensity]
	)
	...
	prompt += (
		"  \"narration\": \"Any environmental event, action outcome, or sensory narration that results from the player's input. MUST be written in the objective third-person (e.g. '%s glances around', never 'I glance around') and progress the scene. Can be empty if there is no narrative event.\",\n"
		% char_name
	)
...
static func get_starters_selection_prompt(
	campaign_title: String, clusters: Array, player_character: Dictionary = {}
) -> String:
	var prompt = ""
	prompt += "=== STORY ARCHITECT / DUNGEON MASTER SYSTEM INSTRUCTIONS ===\n"
	...
	for i in range(clusters.size()):
		...
		if not lr.id.is_empty():
			prompt += (
				"- Associated Lore/Scene: ID: %s | Title: %s | Info: %s\n"
...
static func get_emotion_reflection_prompt(
	...
	prompt += (
		"You ARE the character: %s. You are reflecting on a new event or change in your environment.\n\n"
		% char_name
	)
	...
	prompt += "INSTRUCTIONS:\n"
	prompt += (
		"Reflect on how this new event/narration affects your emotions and your relationship with the player from your perspective as %s.\n"
		% char_name
	)
