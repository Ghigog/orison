...
static func compile_vault(
	vault_path: String, custom_mappings: Dictionary = {}, progress_callback: Callable = Callable()
) -> Dictionary:
...
func run(
	vault_path: String, custom_mappings: Dictionary, progress_callback: Callable
) -> Dictionary:
	...
	_compiled_data = {"knowledge_graph": {"nodes": {}, "edges": []}, "writing_style": ""}
	...
	for node_id in char_nodes.keys():
		...
		_compiled_data["characters"][node_id] = {
			...
			"base_intensity":
			float(props.get("base_emotion_intensity", props.get("base_intensity", -1.0))),
	...
	while file_name != "":
		if dir.current_is_dir():
			...
			if not file_name.begins_with("."):
				_scan_dir_recursive(
					dir_path.path_join(file_name), file_list, image_list, audio_list
				)
		else:
...
func _process_nodes_first_pass():
	for file_path in _file_list:
		...
		if not is_mapped:
			var is_scene_path = (
				file_path_lower.contains("/scene")
				or file_path_lower.contains("/story")
				or file_path_lower.contains("/chapter")
				or file_path_lower.contains("/event")
				or file_path_lower.contains("/quest")
				or file_path_lower.contains("/plot")
			)
			var is_scene_name = (
				file_basename.to_lower()
				in [
					"start",
					"beginning",
					"intro",
			...
			if (
				last_folder.contains("location")
				or last_folder.contains("environment")
				or last_folder.contains("world")
				or last_folder.contains("env")
				or last_folder.contains("map")
				or last_folder.contains("place")
				or last_folder.contains("setting")
			):
				is_location_path = true
			elif (
				last_folder.contains("character")
				or last_folder.contains("npc")
				or last_folder.contains("entity")
				or last_folder.contains("person")
				or last_folder.contains("people")
			):
				is_character_path = true
			else:
				# Fall back to matching anywhere in the folder/file path
				if (
					relative_folder_lower.contains("location")
					or relative_folder_lower.contains("environment")
					or relative_folder_lower.contains("world")
					or relative_folder_lower.contains("env")
					or relative_folder_lower.contains("map")
					or relative_folder_lower.contains("place")
					or relative_folder_lower.contains("setting")
					or file_path_lower.contains("/location")
					or file_path_lower.contains("/environment")
					or file_path_lower.contains("/world")
					or file_path_lower.contains("/env")
					or file_path_lower.contains("/map")
					or file_path_lower.contains("/place")
					or file_path_lower.contains("/setting")
				):
					is_location_path = true
				elif (
					relative_folder_lower.contains("character")
					or relative_folder_lower.contains("npc")
					or relative_folder_lower.contains("entity")
					or relative_folder_lower.contains("person")
					or relative_folder_lower.contains("people")
					or file_path_lower.contains("/character")
					or file_path_lower.contains("/npc")
					or file_path_lower.contains("/entity")
					or file_path_lower.contains("/person")
					or file_path_lower.contains("/people")
				):
					is_character_path = true
			...
			if (
				...
				type in ["character", "npc", "fauna", "flora", "creature", "monster", "bestiary"]
				or is_character_path
			):
				type = "character"
		else:
		...
		match type:
			"character", "npc":
				...
				for key in img_keys:
					if fm.has(key) and fm[key] is String and not fm[key].strip_edges().is_empty():
						...
						for img_path in _image_list:
							if (
								img_path.get_file().to_lower() == val_base
								or img_path.to_lower().ends_with(val.to_lower())
							):
					...
					while true:
						...
						if ext in ["png", "jpg", "jpeg"]:
							...
							for img_path in _image_list:
								if (
									img_path.get_file().to_lower() == val_base
									or img_path.to_lower().ends_with(img_ref.to_lower())
								):
									character_image_path = img_path
									break
						if not character_image_path.is_empty():
				...
				if character_image_path.is_empty():
					...
					while true:
						...
						if body.substr(close_bracket + 1, 1) == "(":
							...
							var img_ref = (
								body
								. substr(close_bracket + 2, close_paren - (close_bracket + 2))
								. strip_edges()
							)
							var ext = img_ref.get_extension().to_lower()
							if ext in ["png", "jpg", "jpeg"]:
				...
				if character_image_path.is_empty():
					...
					if character_image_path.is_empty():
						for img_path in _image_list:
							var img_basename_lower = img_path.get_file().get_basename().to_lower()
							if (
								img_basename_lower.begins_with(node_id_lower)
								or img_basename_lower.begins_with(label_lower)
							):
								character_image_path = img_path
								break
					...
					if character_image_path.is_empty():
						for img_path in _image_list:
							var img_basename_lower = img_path.get_file().get_basename().to_lower()
							if (
								img_basename_lower.contains(node_id_lower)
								or img_basename_lower.contains(label_lower)
							):
								character_image_path = img_path
								break
				...
				if not character_image_path.is_empty():
					...
					if copy_err == OK:
						print(
							"[VaultCompiler] Successfully copied image for ",
							node_id,
							" to ",
							target_path
						)
						...
						printerr(
							"[VaultCompiler] Failed to copy image for ",
							node_id,
							" error: ",
							copy_err
						)
				...
				if graph_manager.has_node(node_id):
					...
					props["base_intensity"] = float(
						fm.get("base_emotion_intensity", fm.get("base_intensity", -1.0))
					)
					...
					for key in voice_keys:
						if (
							fm.has(key)
							and fm[key] is String
							and not fm[key].strip_edges().is_empty()
						):
							voice_val = fm[key]
							break
					var target_voice_path = ""
					if not voice_val.is_empty():
						target_voice_path = _find_and_copy_asset(
							voice_val, _audio_list, "assets/voices", node_id
						)
			...
			"location":
				...
				if not bgm_val.is_empty():
					target_bgm_path = _find_and_copy_asset(
						bgm_val, _audio_list, "assets/audio", node_id
					)

				# Locate location scenery image reference
				var scene_img_val = ""
				var scene_img_keys = [
					"scenery", "background", "image", "cover", "scenery_image", "bg_image", "bg"
				]
				...
				if not scene_img_val.is_empty():
					target_scene_img_path = _find_and_copy_asset(
						scene_img_val, _image_list, "assets/scenes", node_id
					)
	...
	if graph_manager.get_nodes_by_type("scene").is_empty():
		for file_path in _file_list:
			...
			if (
				...
				or file_path_lower.contains("/concepts/")
				or file_path_lower.contains("/systems/")
				or file_path_lower.contains("/gates/")
				or file_path_lower.contains("/rules/")
				or file_path_lower.contains("/templates/")
				or file_path_lower.contains("/meta/")
			):
				continue
			...
			if graph_manager.has_node(node_id):
				...
				graph_manager.add_node(
					node_id, label, "scene", body.substr(0, 500).strip_edges(), props
				)
			break
...
func _extract_character_writing_style(
	char_id: String, char_name: String, char_body: String, fm: Dictionary
) -> String:
	var traits_prefix = ""
	...
	for file_path in _file_list:
		...
		if (
			base_lower == id_lower
			or base_lower.contains(id_lower)
			or base_lower.contains(char_name.to_lower())
		):
			continue
...
static func _determine_character_properties(fm: Dictionary, file_path: String) -> Dictionary:
	...
	var path_indicates_creature = (
		path_lower.contains("/fauna/")
		or path_lower.contains("/flora/")
		or path_lower.contains("/creature")
		or path_lower.contains("/monster")
		or path_lower.contains("/bestiary")
		or path_lower.contains("/animal")
		or path_lower.contains("/beast")
		or path_lower.get_file().begins_with("fauna_")
		or path_lower.get_file().begins_with("flora_")
	)
	...
	var type_indicates_creature = (
		fm_type.contains("creature")
		or fm_type.contains("monster")
		or fm_type.contains("flora")
		or fm_type.contains("fauna")
		or fm_type.contains("animal")
		or fm_type.contains("beast")
	)
	...
	return {"is_creature": is_creature, "can_speak": can_speak, "humanoid": humanoid}


func _extract_character_data_via_llm(body: String, char_name: String) -> Dictionary:
	var result = {"biography": "", "personality": "", "appearance": "", "gender": "", "goals": ""}

	var prompt = (
		"Extract the following fields from these character notes as JSON:\n"
		+ "{\n"
	...
	var state = {"completed": false, "success_received": false, "response_text": ""}
	...
	LLMClient.send_custom_request(
		prompt, LLMClient.world_builder_model, callback, 1500.0, LLMClient.RequestPriority.LOW, true  # json_mode
	)
	...
	if not data.get("parsing_failed", false):
		...
		push_warning(
			(
				"[VaultCompiler] Failed to parse LLM response JSON for %s. Response was: %s"
				% [char_name, state["response_text"]]
			)
		)
...
func _find_and_copy_asset(
	ref_val: String, search_list: Array[String], target_dir_rel: String, file_prefix: String
) -> String:
	if ref_val.is_empty():
		return ""
	...
	for path in search_list:
		if (
			path.get_file().to_lower() == val_base
			or path.to_lower().ends_with(clean_val.to_lower())
		):
			source_path = path
			break
	...
	if copy_err == OK:
		print(
			"[VaultCompiler] Successfully copied asset from %s to %s" % [source_path, target_path]
		)
		return target_path
	printerr(
		(
			"[VaultCompiler] Failed to copy asset from %s to %s (error %d)"
...
func _embed_all_nodes(campaign_id: String):
	...
	if not is_available:
		push_warning(
			"[VaultCompiler] Embedding model 'nomic-embed-text' not available in Ollama. Skipping semantic embedding generation."
		)
		return
...
func _generate_raptor_summaries(campaign_id: String):
		...
		if cluster.is_empty():
			...
			if _progress_callback.is_valid() and Engine.get_main_loop() != null:
				_progress_callback.call(
					"Generating L1 Summary %d/%d..." % [cluster_idx + 1, num_l1_clusters]
				)
				await Engine.get_main_loop().process_frame

			var state = {"completed": false, "success": false, "response_text": ""}
			var callback = func(success: bool, text: String, _error_msg: String):
	...
	for l2_idx in range(l2_clusters.size()):
		...
		if l1_group.is_empty():
			...
			if _progress_callback.is_valid() and Engine.get_main_loop() != null:
				_progress_callback.call(
					"Generating L2 Summary %d/%d..." % [l2_idx + 1, num_l2_clusters]
				)
				await Engine.get_main_loop().process_frame
	...
	print(
		(
			"[VaultCompiler] RAPTOR summaries generated successfully. L1 count: %d, L2 count: %d"
			% [l1_summary_ids.size(), num_l2_clusters]
		)
	)
	EmbeddingStore.save_store(campaign_id)
