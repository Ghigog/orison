...
func generate_scene(description: String, width: int = 512, height: int = 512) -> Image:
	...
	for layer in range(3):
		...
		for x in range(0, width + 10, 10):
			var y = (
				baseline
				+ sin(x * freq + phase) * amplitude
				+ cos(x * freq * 0.5 + phase * 2.0) * (amplitude * 0.3)
			)
...
func generate_avatar(
	char_name: String, description: String, width: int = 256, height: int = 256
) -> Image:
	if DisplayServer.get_name() == "headless":
		var mock = Image.create(width, height, false, Image.FORMAT_RGBA8)
...
func generate_item(
	item_name: String, description: String, width: int = 128, height: int = 128
) -> Image:
	if DisplayServer.get_name() == "headless":
		var mock = Image.create(width, height, false, Image.FORMAT_RGBA8)
	...
	if (
		...
		or "dagger" in desc_lower
		or "weapon" in desc_lower
	):
		# Draw a simple sword
		...
		"shield" in desc_lower
		or "armor" in desc_lower
		or "breastplate" in desc_lower
		or "helm" in desc_lower
	):
		# Draw a shield crest
		icon_points = PackedVector2Array(
			[
		...
		or "vial" in desc_lower
		or "flask" in desc_lower
		or "elixir" in desc_lower
	):
