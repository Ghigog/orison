...
func process_response_tags(char_id: String, tags: Dictionary) -> void:
	...
	var rapport_delta = clamp(
		float(tags.get("rapport_delta", tags.get("affinity_delta", 0.0))), -0.2, 0.2
	)

	# Validate emotion type
	var valid_emotions = [
		"serenity", "joy", "sadness", "anger", "fear", "trust", "disgust", "surprise"
	]
...
func deduce_base_emotion_if_needed(char_id: String, char_data: Dictionary) -> void:
	...
	if biography.strip_edges().is_empty():
		CampaignState.add_emotion_event(
			char_id, "serenity", 0.5, "player", "Default baseline (no biography provided).", 0.0
		)
		return
	...
	LLMClient.send_custom_request(
		...
		func(success: bool, response_text: String, error_msg: String):
			...
			CampaignState.add_emotion_event(
				char_id, deduced_emo, deduced_int, "player", reason, 0.0
			)
			CampaignState.save()

			# Notify UI of updated visuals and sidebar refresh
			character_visual_update_requested.emit(
				char_id, deduced_emo, character.get("affinity", 0.0)
			)
	...
	LLMClient.send_custom_request(
		...
		func(success: bool, response_text: String, error_msg: String):
			...
			if not emotional_update.is_empty():
				...
				var delta_val = clamp(
					float(
						emotional_update.get(
							"rapport_delta", emotional_update.get("affinity_delta", 0.0)
						)
					),
					-0.2,
					0.2
				...
				var msg = (
					"%s reflects on environment: %s (intensity: %.1f) | Rapport: %s"
				...
				var updated_affinity = character.get("affinity", 0.0)
				character_visual_update_requested.emit(
					char_id, emotional_update.get("emotion", "serenity"), updated_affinity
				)
				sidebar_refresh_requested.emit()
	)
