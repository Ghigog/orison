...
const BUDGET_FRACTIONS := {
	"character": [0.30, 0.30, 0.25, 0.15], "world_builder": [0.30, 0.30, 0.30, 0.10]
}
...
static func _report_overflow(role: String, final_tokens: int, limit: int) -> void:
	var msg = (
		"[PromptBuilder] Compiled %s prompt is %d tokens, over the %d token budget. The model will truncate the oldest context, which is the system prompt."
		% [role, final_tokens, limit]
	)
	printerr(msg)
	push_warning(msg)
...
static func get_character_agent_prompt_for_id(
	char_id: String, max_bio_len: int = -1, max_style_len: int = -1
) -> String:
...
static func get_emotion_reflection_prompt_for_id(char_id: String, narration_text: String) -> String:
	...
	return SystemPrompts.get_emotion_reflection_prompt(
		char_name, biography, affinity, active_emotion, intensity, target, context, narration_text
	)
...
func build_prompt(char_id: String, user_prompt: String, is_director_busy: bool = false) -> String:
	...
	var system_prompt_with_rules = get_character_agent_prompt_for_id(
		char_id, max_bio_len, max_style_len
	)
	...
	if all_recent_history.is_empty():
		...
		for idx in range(all_recent_history.size() - 1, -1, -1):
			...
			if accumulated_history_tokens + line_tokens <= remaining_hist_budget:
				...
				print(
					(
						"[PromptBuilder] History budget exceeded. Trimmed older logs (removed %d entries)."
						% [idx + 1]
					)
				)
				break
	...
	trigger_text += (
		"- Parsed Dialogue: "
		+ (parsed.dialogue if not parsed.dialogue.is_empty() else "None")
		+ "\n"
	)
	trigger_text += (
		"- Parsed Action/Context: "
		+ (parsed.action if not parsed.action.is_empty() else "None")
		+ "\n"
	)
...
func build_world_builder_prompt(
	user_prompt: String, active_char_id: String = "", research_findings: String = ""
) -> String:
	...
	if not active_char_id.is_empty():
		...
		var char_name = (
			character.get("name", active_char_id.capitalize())
			if not character.is_empty()
			else active_char_id.capitalize()
		)
		campaign_text += (
			"- Active Conversation Partner: %s (The player is currently addressing and interacting directly with this character. Do not write dialogue for them or have other characters reply to their questions.)\n"
			% char_name
		)
	campaign_text += "\n"
	...
	if all_recent_history.is_empty():
		...
		for idx in range(all_recent_history.size() - 1, -1, -1):
			...
			if accumulated_history_tokens + line_tokens <= remaining_hist_budget:
				...
				print(
					(
						"[PromptBuilder] History budget exceeded in DM prompt. Trimmed older logs (removed %d entries)."
						% [idx + 1]
					)
				)
				break
...
func build_react_prompt(
	user_prompt: String, active_char_id: String, research_history: Array
) -> String:
	var prompt = ""
