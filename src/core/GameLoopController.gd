...
enum TurnState { IDLE, WORLD_BUILDER_THINKING, CHARACTER_THINKING }
...
func _ready() -> void:
	...
	emotion_engine.character_visual_update_requested.connect(
		func(c_id, emo, aff): character_visual_update_requested.emit(c_id, emo, aff)
	)
	emotion_engine.sidebar_refresh_requested.connect(func(): sidebar_refresh_requested.emit())
...
func start_new_campaign(
	...
	if not has_location:
		...
		compiled.knowledge_graph.nodes[fallback_loc_id] = {
			"label": fallback_label, "type": "location", "desc": fallback_desc, "properties": {}
		}
	...
	if (
		...
		and custom_mappings.has("player_character")
		and custom_mappings["player_character"] != null
	):
	...
	if (
		custom_mappings != null
		and custom_mappings.has("starting_location_id")
		and custom_mappings["starting_location_id"] != null
	):
	...
	if (
		custom_mappings != null
		and custom_mappings.has("starting_character_id")
		and custom_mappings["starting_character_id"] != null
	):
	...
	if (
		not starting_location_id.is_empty()
		and compiled.knowledge_graph.nodes.has(starting_location_id)
	):
		...
		for node_id in compiled.knowledge_graph.nodes.keys():
			...
			if node.type in ["character", "npc"]:
				...
				for edge in compiled.knowledge_graph.edges:
					...
					if (
						(f == node_id and t == starting_location_id)
						or (t == node_id and f == starting_location_id)
					):
				...
				if not is_associated:
					...
					if (
						desc_lower.contains(label_lower)
						or desc_lower.contains(starting_location_id.to_lower().replace("_", " "))
					):
						is_associated = true
		...
		LLMClient.send_custom_request(
			...
			_on_beginning_generation_completed,
			1500.0,
...
func _on_beginning_generation_completed(
	success: bool, response_text: String, _error_msg: String
) -> void:
	_is_generating_beginning = false
	_current_turn_state = TurnState.IDLE
	turn_state_changed.emit(_current_turn_state)
	...
	if (
		clean_response.is_empty()
		or text_response.contains("```")
		or (text_response == response_text and not response_text.contains("{"))
	):
...
func send_player_input(input_text: String) -> void:
	...
	var npc_prompt = await prompt_builder.build_prompt(
		active_character_id, input_text, _is_director_running
	)
	LLMClient.send_custom_stream_request(
		npc_prompt,
		LLMClient.character_model,
		func(chunk: String): stream_parser.ingest_chunk(chunk),
...
func _on_npc_stream_completed(full_response: String) -> void:
	...
	if parsed.get("parsing_failed", false):
		error_message_logged.emit(
			"The DM was unable to format this response. [url=retry]Click to retry.[/url]"
		)
		_current_turn_state = TurnState.IDLE
		turn_state_changed.emit(_current_turn_state)
		input_disabled_changed.emit(false)
	...
	if not active_character_id.is_empty():
		...
		if not character.is_empty():
			...
			var current_emotion = (
				emotions[-1].get("emotion", "serenity") if not emotions.is_empty() else "serenity"
			)
	...
	if not emotional_update.is_empty():
		var character = CampaignState.get_character(active_character_id)
		var friendly_char_name = (
			character.get("name", active_character_id.capitalize())
			if not character.is_empty()
			else active_character_id.capitalize()
		)
		...
		var delta_val = clamp(
			float(
				emotional_update.get("rapport_delta", emotional_update.get("affinity_delta", 0.0))
			),
			-0.2,
			0.2
		)
		...
		var delta_str = (
			"+%.2f" % delta_val
			if delta_val > 0
			else ("%.2f" % delta_val if delta_val < 0 else "0.0")
		)
		var msg = (
			"%s feels: %s (intensity: %.1f) | Rapport: %s"
...
func retry_last_input() -> void:
	...
	var npc_prompt = await prompt_builder.build_prompt(
		active_character_id, _last_player_input, _is_director_running
	)
	input_disabled_changed.emit(true)
	EventBus.turn_started.emit()
			...
			if update is Dictionary:
				...
				if not item_id.is_empty():
					if action == "add":
						...
						system_message_logged.emit(
							"Added to Inventory: %s x%d" % [item_id.capitalize(), qty]
						)
					elif action == "remove":
						CampaignState.remove_from_inventory(active_character_id, item_id, qty)
						system_message_logged.emit(
							"Removed from Inventory: %s x%d" % [item_id.capitalize(), qty]
						)
...
func _trigger_background_director() -> void:
	...
	var dm_prompt = await prompt_builder.build_world_builder_prompt(
		_last_player_input, active_character_id, research_findings
	)
	EventBus.director_prompt_generated.emit(dm_prompt)
	LLMClient.send_custom_request(
		dm_prompt,
		LLMClient.world_builder_model,
		_on_background_director_completed,
		1500.0,
		LLMClient.RequestPriority.LOW,
		false,
		LLMClient.ROLE_WORLD_BUILDER
	)


# Inner class carrier to wrap custom requests in an awaitable co-routine
class ReActSignalCarrier:
	extends RefCounted
...
func _run_director_react_loop(user_prompt: String, active_char_id: String) -> String:
	...
	for i in range(max_iterations):
		...
		var prompt = prompt_builder.build_react_prompt(
			user_prompt, active_char_id, research_history
		)
			...
			"search_knowledge_graph":
				...
				if not query.is_empty():
					...
					if observation.strip_edges().is_empty():
						observation = (
							"No relevant entities or lore found in the knowledge graph for query: "
							+ query
						)
				else:
				...
				if not character_name.is_empty():
					...
					if character.is_empty():
						...
						observation = (
							...
							+ character.get("name", char_id.capitalize())
							+ ":\n"
						)
						...
						if not character.get("gender", "").is_empty():
							observation += (
								"- Gender/Pronouns: " + character.get("gender", "") + "\n"
							)
						observation += (
							"- Relationship: %s (Affinity: %.2f)\n"
							% [
								CharacterProfile.get_relationship_label(
									character.get("affinity", 0.0)
								),
								character.get("affinity", 0.0)
							]
						)
				...
				if not location_name.is_empty():
					...
					if node.is_empty():
						...
						observation = (
							...
							+ node.get("label", loc_id.capitalize())
							+ " ("
							+ node.get("type", "location")
							+ "):\n"
						)
						observation += (
							"- Description: " + node.get("desc", "No description.") + "\n"
						)
			...
			"get_relationship":
				...
				if not entity_a.is_empty() and not entity_b.is_empty():
					...
					if found_edges.is_empty():
						observation = (
							"No relationship edges found between " + entity_a + " and " + entity_b
						)
					else:
						observation = (
							"Relationships between " + entity_a + " and " + entity_b + ":\n"
						)
						for edge in found_edges:
							var from_lbl = graph_manager.get_node(edge.get("from", "")).get(
								"label", edge.get("from", "")
							)
							var to_lbl = graph_manager.get_node(edge.get("to", "")).get(
								"label", edge.get("to", "")
							)
							observation += (
								"- "
		...
		research_history.append(
			{"thought": thought, "action": action, "args": args, "observation": observation}
		)
...
func _on_background_director_completed(
	success: bool, response_text: String, error_msg: String
) -> void:
	...
	if (
		clean_narration.is_empty()
		or narration_text.contains("```")
		or (narration_text == response_text and not response_text.contains("{"))
	):
...
func get_nearby_character_ids() -> Array[String]:
	...
	for char_id in char_ids:
		...
		for neighbor_id in graph_manager.get_neighbors(active_location):
			...
			if (
				neighbor_node
				and neighbor_node.get("type", "") in ["location", "environment", "gate"]
			):
		...
		if not is_nearby:
			...
			if (
				bio.contains(loc_label)
				or bio.contains(active_location.to_lower().replace("_", " "))
			):
				is_nearby = true
