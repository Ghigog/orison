...
func start(
	...
	print(
		(
			"[LLMStream] Starting request to model '%s' (ctx: %d, timeout: %ds, prompt length: %d chars)%s"
			% [model_name, num_ctx, int(timeout_seconds), prompt.length(), mode_str]
		)
	)
...
func _process(delta: float) -> void:
	...
	if status == HTTPClient.STATUS_DISCONNECTED:
		...
		if not _stream_started:
			...
			print(
				"[LLMStream] Connected. Sending POST /api/generate for model '%s'..." % model_name
			)
			...
			if _has_entered_requesting:
				print(
					(
						"[LLMStream] Status returned to CONNECTED after requesting. Completing. (chunks: %d, response length: %d)"
						% [_chunk_count, _accumulated_response.length()]
					)
				)
				_complete()
			else:
		...
		if _client.has_response():
			...
			if _response_code == -1:
				_response_code = code
				print(
					"[LLMStream] Received HTTP %d from server for model '%s'." % [code, model_name]
				)
			if code != 200:
...
func _complete() -> void:
	...
	if _accumulated_response.strip_edges().is_empty():
		print(
			(
				"[LLMStream] WARNING: Model '%s' completed with EMPTY response after %.1fs. (HTTP %d, chunks received: %d)"
				% [model_name, _elapsed_time, _response_code, _chunk_count]
			)
		)
	else:
		print(
			(
				"[LLMStream] Completed for model '%s'. Response: %d chars, %d chunks, %.1fs elapsed."
				% [model_name, _accumulated_response.length(), _chunk_count, _elapsed_time]
			)
		)
