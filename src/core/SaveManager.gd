					...
					list.append(
						{
							...
							"title": title,
							"last_played":
							metadata.get("saved_at", data.adventure_meta.get("last_played", "")),
...
static func save_campaign(campaign_id: String, state_data: Dictionary) -> Error:
	...
	if err != OK:
		printerr(
			"Failed to rename temporary save file to final: ", final_path, " (Error: ", err, ")"
		)
		return err
