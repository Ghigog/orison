...
func _ready() -> void:
	...
	var start_params = campaign_graph_view.get_node_or_null(
		"InspectorPanel/MarginContainer/ScrollContainer/VBoxContainer/StartParamsGroup"
	)
	if start_params:
		start_params.visible = false
	var separator = campaign_graph_view.get_node_or_null(
		"InspectorPanel/MarginContainer/ScrollContainer/VBoxContainer/HSeparator2"
	)
	if separator:
	...
	if overlay and modal_panel:
		...
		modal_panel.item_rect_changed.connect(
			func(): modal_panel.pivot_offset = modal_panel.size / 2.0
		)

		var entrance_tween = create_tween().set_parallel(true)
		(
			entrance_tween
			. tween_property(overlay, "color:a", target_bg_alpha, 0.3)
			. set_trans(Tween.TRANS_CUBIC)
			. set_ease(Tween.EASE_OUT)
		)
		(
			entrance_tween
			. tween_property(modal_panel, "modulate:a", 1.0, 0.3)
			. set_trans(Tween.TRANS_CUBIC)
			. set_ease(Tween.EASE_OUT)
		)
		(
			entrance_tween
			. tween_property(modal_panel, "scale", Vector2(1.0, 1.0), 0.3)
			. set_trans(Tween.TRANS_CUBIC)
			. set_ease(Tween.EASE_OUT)
		)
...
func _on_close_pressed() -> void:
	...
	if overlay and modal_panel:
		...
		(
			exit_tween
			. tween_property(overlay, "color:a", 0.0, 0.25)
			. set_trans(Tween.TRANS_CUBIC)
			. set_ease(Tween.EASE_IN)
		)
		(
			exit_tween
			. tween_property(modal_panel, "modulate:a", 0.0, 0.25)
			. set_trans(Tween.TRANS_CUBIC)
			. set_ease(Tween.EASE_IN)
		)
		(
			exit_tween
			. tween_property(modal_panel, "scale", Vector2(0.95, 0.95), 0.25)
			. set_trans(Tween.TRANS_CUBIC)
			. set_ease(Tween.EASE_IN)
		)
