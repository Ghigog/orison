...
const RELATION_TYPES = [
	"connected_to", "associated_with", "relationships", "leads_to", "friend", "enemy", "ally"
]
...
func initialize(
	graph_data: Dictionary, start_loc_id: String = "", start_char_id: String = ""
) -> void:
...
func _compute_layout() -> Dictionary:
	...
	var rows = {}
	for loc_id in locations:
		rows[loc_id] = {"location": [loc_id], "lore": [], "scene": [], "character": []}

	var global_row = {"location": [], "lore": [], "scene": [], "character": []}
	...
	var has_global = (
		not global_row.lore.is_empty()
		or not global_row.scene.is_empty()
		or not global_row.character.is_empty()
	)
	if has_global:
	...
	var col_x = {"location": 320.0, "lore": 520.0, "scene": 720.0, "character": 920.0}
...
func _populate_connections_list() -> void:
	...
	for edge in node_edges:
		...
		item.delete_pressed.connect(func(): _remove_edge(edge))
...
func _on_connection_request(
	...
	var edge = {
		"from": str(from_node), "to": str(to_node), "relation": "connected_to", "weight": 1.0
	}
...
func _on_add_connection_pressed() -> void:
	...
	# Add edge
	var edge = {"from": _selected_node_id, "to": target_id, "relation": relation, "weight": 1.0}
...
func _apply_filters() -> void:
	...
	for child in graph_edit.get_children():
		...
		if node_name.begins_with("header_"):
			...
			match header_type:
				"location":
					header_visible = show_location
				"lore":
					header_visible = show_lore
				"scene":
					header_visible = show_scene
				"character":
					header_visible = show_character
		...
		match node_type:
			"location":
				category_visible = show_location
			"lore":
				category_visible = show_lore
			"scene":
				category_visible = show_scene
			"character":
				category_visible = show_character
			_:
		...
		var matches_search = (
			...
			or node_name.to_lower().contains(query)
			or desc.contains(query)
		)
