...
func _ready() -> void:
	...
	modal_card.item_rect_changed.connect(func(): modal_card.pivot_offset = modal_card.size / 2.0)
	...
	(
		entrance_tween
		. tween_property(overlay, "color:a", 0.55, 0.3)
		. set_trans(Tween.TRANS_CUBIC)
		. set_ease(Tween.EASE_OUT)
	)
	(
		entrance_tween
		. tween_property(modal_card, "modulate:a", 1.0, 0.3)
		. set_trans(Tween.TRANS_CUBIC)
		. set_ease(Tween.EASE_OUT)
	)
	(
		entrance_tween
		. tween_property(modal_card, "scale", Vector2(1.0, 1.0), 0.3)
...
func _apply_tutorial_styling() -> void:
	...
	block1.text = (
		"By default, Orison draws simple retro artwork automatically — [b]no setup needed at all[/b].\n\n"
		...
		+ (
			"[color=#%s]Your data never leaves your computer. Everything runs locally, for free.[/color]"
			% [muted_hex]
		)
	)
	...
	block2.text = (
		"[b][color=#%s]Step 1 — Install Draw Things[/color][/b]\n" % [accent_hex]
	...
	block3.text = (
		"WebUI Forge is the same technology that runs inside Draw Things, but as a web page on your own "
	...
	block4.text = (
		"[b][color=#%s]Step 1 — Download WebUI Forge[/color][/b]\n" % [accent_hex]
		+ (
		...
		+ (
			"[color=#%s]Note: WebUI Forge on Windows works best with an NVIDIA GPU. If you only have an "
			% [muted_hex]
		)
	...
	block5.text = (
		"[b]Do I need a special graphics card?[/b]\n"
		...
		+ (
			"It tells Orison where to find the image generator. The default [color=#%s]http://127.0.0.1:7860[/color] "
			% [accent_hex]
		)
...
func close() -> void:
	...
	exit_tween.tween_property(overlay, "color:a", 0.0, 0.25).set_trans(Tween.TRANS_CUBIC).set_ease(
		Tween.EASE_IN
	)
	(
		exit_tween
		. tween_property(modal_card, "modulate:a", 0.0, 0.25)
		. set_trans(Tween.TRANS_CUBIC)
		. set_ease(Tween.EASE_IN)
	)
	(
		exit_tween
		. tween_property(modal_card, "scale", Vector2(0.92, 0.92), 0.25)
		. set_trans(Tween.TRANS_CUBIC)
		. set_ease(Tween.EASE_IN)
	)
