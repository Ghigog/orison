...
func refresh_campaign_list() -> void:
	...
	if saves.is_empty():
		...
		for save in saves:
			...
			item.setup(save)
			item.selected.connect(func(campaign_id): campaign_selected.emit(campaign_id))
			item.delete_requested.connect(func(campaign_id): _prompt_delete_campaign(campaign_id))
