...
func _on_pc_magic_wand_pressed() -> void:
	...
	var on_failed = func(error_msg: String): _on_vision_description_completed(false, "", error_msg)

	LLMClient.send_custom_vision_stream_request(
		prompt, avatar_path, LLMClient.world_builder_model, on_chunk, on_completed, on_failed
	)


func _on_vision_description_completed(
...
func _on_pc_avatar_magic_wand_pressed() -> void:
	...
	img_gen.generate_asset(pc_name_input.text.strip_edges(), physical_desc, "avatar", target_avatar)
...
func _set_wand_status(message: String, is_error: bool) -> void:
	...
	if is_error:
		...
		var muted_c = ThemeManager.color_text
		muted_c.a = 0.65
...
func _on_next_pressed() -> void:
	...
	if campaign_art_style_dropdown:
		_player_character["art_style"] = campaign_art_style_dropdown.get_item_text(
			campaign_art_style_dropdown.selected
		)
	else:
