...
func initialize(char_id: String) -> void:
	...
	if bio.is_empty():
		bio = character.get(
			"physical_description", character.get("personality", "No details available.")
		)
	bio_label.text = bio
	...
	if not emotions.is_empty():
		...
		emotion_label.text = (
			"Feeling: %s (Intensity: %.1f)" % [emotion_name.capitalize(), intensity]
		)
		emotion_label.add_theme_color_override(
			"font_color", ThemeManager.get_emotion_color(emotion_name)
		)
