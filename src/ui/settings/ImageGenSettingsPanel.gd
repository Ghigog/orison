...
func _ready() -> void:
	...
	image_gen_provider_dropdown.selected = (
		0 if LLMClient.image_gen_provider == "automatic1111" else 1
	)
	...
	image_gen_enable_check.toggled.connect(
		func(pressed):
...
func apply_settings() -> void:
	...
	LLMClient.image_gen_provider = (
		"automatic1111" if image_gen_provider_dropdown.selected == 0 else "comfyui"
	)
...
func _on_test_connection_pressed() -> void:
	...
	probe_http.request_completed.connect(
		func(result: int, response_code: int, _headers: PackedStringArray, _body: PackedByteArray):
			...
			if result == HTTPRequest.RESULT_SUCCESS and response_code == 200:
				connection_status_label.add_theme_color_override(
					"font_color", Color.html("#52cc7a")
				)  # Green
				connection_status_label.text = "✅ Local AI image generator detected and ready!"
			else:
				connection_status_label.add_theme_color_override(
					"font_color", Color.html("#e05353")
				)  # Red
				connection_status_label.text = (
					"❌ Not found at %s. Is Draw Things/A1111 running?" % gen_url
				)
