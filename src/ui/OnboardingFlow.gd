...
signal adventure_started(
	campaign_id: String, title: String, vault_path: String, custom_mappings: Dictionary
)
...
const CATEGORIES = [
	{"id": "scene", "label": "Scenes & Story", "icon": "🎬", "color": Color(1.0, 0.373, 0.220)},
	{
		"id": "character",
	...
	{"id": "location", "label": "Locations & World", "icon": "📍", "color": Color(0.96, 0.62, 0.04)},
	{"id": "lore", "label": "Lore & Systems", "icon": "📚", "color": Color(0.624, 0.525, 0.753)}
]
...
func _ready() -> void:
	...
	review_screen.back_pressed.connect(
		func():
...
func _on_campaign_crafted(
	title: String, campaign_id: String, vault_path: String, use_sample: bool
) -> void:
	...
	if campaign_exists:
		overwrite_dialog.dialog_text = (
			"An adventure with the ID '%s' already exists. Overwriting will delete all previous saves and progress for this adventure. Do you want to proceed?"
			% campaign_id
		)
...
func _start_background_setup() -> void:
	...
	var on_progress = func(current: int, total: int):
		...
		if character_creator:
			character_creator.set_compilation_progress(
				"⚙️ Scanning vault files (%d/%d)..." % [current, total], false, _warmup_success
			)
	...
	if not _background_compile_completed:
		...
		if not _background_compile_completed:
			if not _background_compilation_done:
				...
				loading_text.text = (
					"Warming up local LLM models (%d remaining)..." % _pending_warmups
				)
...
func _generate_adventure_hooks() -> void:
	...
	var prompt = SystemPrompts.get_starters_selection_prompt(
		campaign_title, _selected_clusters, _player_character
	)
	...
	var on_completed = func(response_text: String):
		...
		if not (starters is Array) or starters.is_empty():
			...
			_on_hooks_generation_failed(
				(
					"Pass 1 (selection) failed to return a valid starters list. Response: "
	...
	print(
		(
			"[OnboardingFlow] Requesting Pass 1 starter selection from model '%s' (prompt: %d chars)..."
			% [active_model, prompt.length()]
		)
	)
	LLMClient.send_custom_stream_request(
		prompt, active_model, on_chunk, on_completed, on_failed, 300.0, true
	)
...
func _generate_next_narration() -> void:
	...
	var loc_node = nodes.get(
		loc_id, {"id": loc_id, "name": loc_id.capitalize(), "desc": "A starting location."}
	)
	var char_node = nodes.get(
		char_id, {"id": char_id, "name": char_id.capitalize(), "desc": "A starting character."}
	)
	...
	var on_completed = func(response_text: String):
		...
		if narration_text.is_empty():
			narration_text = (
				"%s. %s is here at %s."
				% [
					selection.get("concept", "A strange event unfolds"),
					char_node.get("name", "Someone"),
					loc_node.get("name", "someplace")
				]
			)
	...
	var on_failed = func(error_msg: String):
		...
		print(
			(
				"[OnboardingFlow] Narration pass FAILED for hook %d. Error: %s"
				% [_current_hook_idx + 1, error_msg]
			)
		)
	...
	print(
		(
			"[OnboardingFlow] Requesting Pass 2 narration for hook %d/3 from model '%s' (prompt: %d chars)..."
			% [_current_hook_idx + 1, active_model, prompt.length()]
		)
	)
	LLMClient.send_custom_stream_request(
		prompt, active_model, on_chunk, on_completed, on_failed, 300.0, true
...
func _on_hooks_generation_failed(error_msg: String) -> void:
	...
	if err_label:
		err_label.text = (
			"⚠️ Hook generation failed (model: %s — %s).\nUsing offline fallbacks instead."
			% [active_model, error_msg]
		)
...
func _process(delta: float) -> void:
	...
	if _loading_phase == "connecting":
		...
		var current_model = (
			...
			if _generated_starters.is_empty()
			else LLMClient.character_model
		)
		...
		if _warmup_success and _background_compile_completed:
			...
			loading_text.text = (
				"Evaluating notes and preparing adventure hooks...\nModel: %s (num_ctx: 8192)\n\n%s\nStatus: Performing prompt evaluation (pre-processing)\nElapsed time: %.1fs"
				% [current_model, _get_moving_dot_bar(_loading_dots_frame), _loading_elapsed]
			)
		else:
			if loading_title:
				loading_title.text = "WAKING UP LOCAL LLM" + dots
			loading_text.text = (
				"Loading '%s' into system memory\nModel: %s (num_ctx: 8192)\n\n%s\nStatus: Allocating VRAM/RAM (can take up to a minute)\nElapsed time: %.1fs"
				% [
					current_model,
					current_model,
					_get_moving_dot_bar(_loading_dots_frame),
...
func _build_connected_starting_clusters() -> Array:
	...
	for loc in locations:
		loc_connections[loc.id] = {"characters": [], "lore": []}
	...
	loc_candidates.sort_custom(
		func(a, b):
			var a_conns = (
				loc_connections[a.id].characters.size() + loc_connections[a.id].lore.size()
			)
			var b_conns = (
				loc_connections[b.id].characters.size() + loc_connections[b.id].lore.size()
			)
	...
	while selected_locs.size() < 3:
		if selected_locs.size() > 0:
			...
			selected_locs.append(
				{"id": "", "name": "Unknown Location", "desc": "A mysterious starting point."}
			)
	...
	for loc in selected_locs:
		...
		if candidate_chars.is_empty():
			candidate_chars.append(
				{"id": "companion", "name": "Companion", "desc": "A quiet traveler assisting you."}
			)
	...
	for i in range(3):
		...
		match i:
			0:
				...
				desc = (
					"A sudden, freezing breeze sweeps through %s. A quiet whisper calls out your name."
					% loc.name
				)
				narration = (
					"The air grows thin and cold as you arrive at %s. A quiet, raspy whisper echoes from the shadows, calling your name. Standing nearby, %s notices the sudden drop in temperature, their hand instinctively reaching for their weapon as they look around in quiet alarm."
					% [loc.name, ch.name]
				)
			1:
				title = "The Burning Sigil"
				desc = (
					"A flash of crimson light erupts in %s, carving a glowing, hot sigil into the stone."
					% loc.name
				)
				narration = (
					"A sharp hiss echoes through %s as a brilliant crimson sigil begins to glow on a nearby stone surface. The rock cracks under the sudden intense heat. %s stares at the burning mark, face pale in the red light, whispering that this is a sign they had hoped never to see again."
					% [loc.name, ch.name]
				)
			2:
				title = "The Shattered Mirror"
				desc = (
					"A loud crack splits the air in %s, leaving behind a shimmering rift that distorts reality."
					% loc.name
				)
				narration = (
					"A deafening sound like shattering glass reverberates through %s. In its wake, a vertical shimmer hangs in the air, refracting light like a warped lens. %s takes a step back, warning you that the fabric of this place has just fractured, and whatever is on the other side is starting to look back."
					% [loc.name, ch.name]
				)
...
func _on_review_next_pressed() -> void:
	...
	if (
		not char_id.is_empty()
		and _compiled_data.has("characters")
		and _compiled_data["characters"].has(char_id)
	):
	...
	(
		tween
		. tween_property(self, "modulate:a", 0.0, ThemeManager.duration_slow)
...
func _generate_sample_vault(path: String) -> void:
	...
	_write_file(
		path.path_join("elara_the_wise.md"),
		"""---
type: character
name: Elara the Wise
base_affinity: 0.2
...
"""
	...
	_write_file(
		path.path_join("valen_the_ranger.md"),
		"""---
type: character
...
"""
	...
	_write_file(
		path.path_join("phandalin_town_square.md"),
		"""---
type: location
...
"""
	...
	_write_file(
		path.path_join("crypt_of_shadows.md"),
		"""---
type: location
...
"""
	...
	# 5. Intro Scene File
	_write_file(
		path.path_join("intro_the_journey_begins.md"),
		"""---
type: scene
...
"""
	...
	_write_file(
		path.path_join("writing_style.md"),
		"""---
type: lore
