...
func _ready() -> void:
	...
	game_loop_controller.character_visual_update_requested.connect(
		_on_controller_character_visual_update_requested
	)
	game_loop_controller.character_reaction_requested.connect(
		_on_controller_character_reaction_requested
	)
	...
	_display_system_message(
		"Welcome to Orison! Import an Obsidian vault folder to begin your adventure."
	)

	_ensure_focus_mode(self)

	print(
		"[SYSTEM] MainViewport loaded. Renderer: ",
...
func _on_controller_stream_chunk_logged(sender: String, word: String) -> void:
	if _stream_is_first_chunk:
		...
		var sender_id = (
			"narrator" if game_loop_controller.stream_parser.stream_zone == "narration" else sender
		)
...
func _on_controller_turn_state_changed(state: int) -> void:
	...
	if (
		state == GameLoopController.TurnState.WORLD_BUILDER_THINKING
		or state == GameLoopController.TurnState.CHARACTER_THINKING
	):
...
func _on_controller_character_visual_update_requested(
	char_id: String, emotion: String, affinity: float
) -> void:
...
func _set_input_disabled(disabled: bool) -> void:
	...
	if game_loop_controller:
		...
		if (
			state == GameLoopController.TurnState.CHARACTER_THINKING
			or state == GameLoopController.TurnState.WORLD_BUILDER_THINKING
		):
			can_cancel = true
...
func _append_message(
	sender: String, text: String, type: String = "chat", is_temporary: bool = false
) -> void:
	_displayed_messages.append(
		{"sender": sender, "text": text, "type": type, "is_temporary": is_temporary}
	)
	_rebuild_dialogue_text()
...
func _populate_chat_from_history() -> void:
	...
	if not history.is_empty():
		...
		if not intro_narration.is_empty() and not first_entry_is_intro:
			...
			for i in range(logs.size()):
				if (
					logs[i].content == history[0].content
					and logs[i].timestamp == history[0].timestamp
				):
			...
			if (
				intro_index != -1
				and history_start_index != -1
				and history_start_index > intro_index + 1
			):
		...
		_append_message(
			"system",
			"Loaded campaign '" + CampaignState.get_campaign_meta("title") + "'. Ready.",
			"system"
		)
...
func _display_error_message(msg: String) -> void:
	...
	if (
		"connect" in clean_msg.to_lower()
		or "status: 0" in clean_msg.to_lower()
		or "cant_connect" in clean_msg.to_lower()
	):
		...
		"stalled" in clean_msg.to_lower()
		or "timeout" in clean_msg.to_lower()
		or "interrupted" in clean_msg.to_lower()
	):
...
func _on_toggle_sidebar_pressed() -> void:
	...
	(
		_sidebar_tween
		. tween_property(
			sidebar_container, "offset_left", target_offset_left, ThemeManager.duration_normal
		)
	...
	(
		_sidebar_tween
		. tween_property(
			sidebar_container, "offset_right", target_offset_right, ThemeManager.duration_normal
		)
		. set_trans(ThemeManager.trans_default)
		. set_ease(ThemeManager.ease_default)
	)
...
func _update_memory_ui() -> void:
	...
	if short_term_memory_label:
		short_term_memory_label.text = (
			"[color=%s][b]Short-Term:[/b][/color] " % green_color + memory.get("short_term", "None")
		)

	if medium_term_memory_label:
		var mt_text = (
			"[color=%s][b]Medium-Term:[/b][/color] " % yellow_color
			+ memory.get("medium_term", "None")
		)
		if game_loop_controller and not game_loop_controller.active_character_id.is_empty():
			var character = CampaignState.get_character(game_loop_controller.active_character_id)
			if not character.is_empty():
				var char_name = character.get(
					"name", game_loop_controller.active_character_id.capitalize()
				)
				var char_mt_list = character.get("medium_term_memories", [])
				if not char_mt_list.is_empty():
					mt_text += (
						" | [color=%s][b]%s Memories:[/b][/color]" % [yellow_color, char_name]
					)
	...
	if long_term_memory_label:
		var lt_text = (
			"[color=%s][b]Long-Term:[/b][/color] " % red_color + memory.get("long_term", "None")
		)
		if game_loop_controller and not game_loop_controller.active_character_id.is_empty():
			var character = CampaignState.get_character(game_loop_controller.active_character_id)
			if not character.is_empty():
				...
				if not char_lt.is_empty():
					lt_text += (
						" | [color=%s][b]%s LTM:[/b][/color] %s" % [red_color, char_name, char_lt]
					)
...
func _on_mind_map_pressed() -> void:
	...
	if modal_scene:
		...
		modal.theme = ThemeManager.active_theme
		add_child(modal)
		modal.closed.connect(func(): _refresh_character_list())
...
func _on_background_generated(output_path: String, _is_placeholder: bool) -> void:
	...
	var expected_path = (
		"user://adventures/%s/generated_assets/scene_%s.png" % [campaign_id, filename]
	)
...
func _fade_in_background(new_texture: Texture2D) -> void:
	...
	(
		_bg_fade_tween
		. tween_property(bg_texture_rect, "modulate:a", 0.0, ThemeManager.duration_normal)
		. set_trans(ThemeManager.trans_default)
		. set_ease(ThemeManager.ease_default)
	)
	_bg_fade_tween.tween_callback(func(): bg_texture_rect.texture = new_texture)
	(
		_bg_fade_tween
		. tween_property(bg_texture_rect, "modulate:a", 1.0, ThemeManager.duration_slow)
		. set_trans(ThemeManager.trans_default)
		. set_ease(ThemeManager.ease_default)
	)
...
func _fade_out_background(duration: float = ThemeManager.duration_normal) -> void:
	...
	(
		_bg_fade_tween
		. tween_property(bg_texture_rect, "modulate:a", 0.0, duration)
		. set_trans(ThemeManager.trans_default)
		. set_ease(ThemeManager.ease_default)
	)
...
func show_toast(message: String, is_error: bool = false, duration: float = 4.0) -> void:
	...
	(
		tween
		. tween_property(toast, "position:x", toast.position.x - 50, ThemeManager.duration_normal)
		. set_trans(ThemeManager.trans_default)
		. set_ease(ThemeManager.ease_default)
	)
	...
	if _active_tasks.is_empty():
		if _loading_indicator.visible:
			...
			tween.tween_property(
				_loading_indicator, "modulate:a", 0.0, ThemeManager.duration_normal
			)
		...
		if not _loading_indicator.visible:
			...
			var tween = create_tween()
			tween.tween_property(
				_loading_indicator, "modulate:a", 1.0, ThemeManager.duration_normal
			)
...
func _on_character_selected(char_id: String) -> void:
	...
	if modal_scene:
		...
		modal.talk_requested.connect(func(id: String): game_loop_controller.select_character(id))
...
func _close_or_toggle_settings() -> void:
	...
	for i in range(children.size() - 1, -1, -1):
		...
		if (
			child.name.ends_with("Modal")
			or child is SettingsModal
			or child is MindMapModal
			or child is CharacterDetailModal
			or child is SaveAsModal
		):
