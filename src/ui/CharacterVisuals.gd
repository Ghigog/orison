	...
	if sprite_rect:
		...
		if texture:
			...
			if sprite_rect.modulate.a < 1.0:
				...
				(
					fade_tween
					. tween_property(sprite_rect, "modulate:a", 1.0, 0.45)
...
func fade_out(duration: float = 0.25) -> void:
	...
	if sprite_rect and sprite_rect.modulate.a > 0.0:
		...
		(
			fade_tween
			. tween_property(sprite_rect, "modulate:a", 0.0, duration)
			. set_trans(Tween.TRANS_CUBIC)
			. set_ease(Tween.EASE_OUT)
		)
...
func fade_in(duration: float = 0.25) -> void:
	...
	if sprite_rect and sprite_rect.modulate.a < 1.0:
		...
		(
			fade_tween
			. tween_property(sprite_rect, "modulate:a", 1.0, duration)
			. set_trans(Tween.TRANS_CUBIC)
			. set_ease(Tween.EASE_OUT)
		)
...
func apply_emotion(emotion: String, _affinity: float) -> void:
	...
	match emotion:
		"anger":
			...
			for i in range(5):
				...
				(
					motion_tween
					. tween_property(sprite_rect, "position:x", _original_position.x + offset, 0.05)
					. set_delay(i * 0.05)
				)
			motion_tween.chain().tween_property(
				sprite_rect, "position:x", _original_position.x, 0.05
			)
		...
		"sadness":
			...
			(
				motion_tween
				. tween_property(sprite_rect, "position:y", _original_position.y + 15.0, 0.4)
		...
		"joy":
			...
			(
				motion_tween
				. tween_property(sprite_rect, "position:y", _original_position.y - 30.0, 0.2)
				. set_trans(Tween.TRANS_QUAD)
				. set_ease(Tween.EASE_OUT)
			)
			(
				motion_tween
				. chain()
				. tween_property(sprite_rect, "position:y", _original_position.y, 0.25)
		...
		"trust":
			...
			(
				motion_tween
				. tween_property(sprite_rect, "scale", Vector2(1.03, 1.03), 0.6)
			...
			(
				motion_tween
				. chain()
				. tween_property(sprite_rect, "scale", Vector2(1.0, 1.0), 0.6)
				. set_trans(Tween.TRANS_SINE)
				. set_ease(Tween.EASE_IN_OUT)
			)
		...
		"disgust":
			...
			for i in range(4):
				...
				(
					motion_tween
					. tween_property(sprite_rect, "position:x", _original_position.x + offset, 0.08)
					. set_delay(i * 0.08)
				)
			motion_tween.chain().tween_property(
				sprite_rect, "position:x", _original_position.x, 0.08
			)
		...
		"surprise":
			...
			(
				motion_tween
				. tween_property(sprite_rect, "scale", Vector2(1.08, 1.08), 0.15)
				. set_trans(Tween.TRANS_QUAD)
				. set_ease(Tween.EASE_OUT)
			)
			(
				motion_tween
				. chain()
				. tween_property(sprite_rect, "scale", Vector2(1.0, 1.0), 0.25)
				. set_trans(Tween.TRANS_SINE)
				. set_ease(Tween.EASE_OUT)
			)
	...
	(
		color_tween
		. tween_property(sprite_rect, "self_modulate", target_color, 0.4)
		. set_trans(Tween.TRANS_QUAD)
		. set_ease(Tween.EASE_OUT)
	)
...
func _hide_reaction() -> void:
	...
	(
		_reaction_tween
		. tween_property(reaction_panel, "modulate:a", 0.0, 0.3)
		. set_trans(Tween.TRANS_QUAD)
		. set_ease(Tween.EASE_OUT)
	)
	_reaction_tween.tween_callback(func(): reaction_panel.visible = false)
...
func _show_reaction(text_content: String) -> void:
	...
	(
		_reaction_tween
		. tween_property(reaction_panel, "modulate:a", 1.0, 0.45)
		. set_trans(Tween.TRANS_CUBIC)
		. set_ease(Tween.EASE_OUT)
	)
