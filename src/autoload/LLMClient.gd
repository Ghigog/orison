...
enum RequestPriority { LOW = 0, HIGH = 1 }
...
func _ready() -> void:
	...
	load_config()
	EventBus.location_changed.connect(
		func(_loc_id: String):
	...
	if status == HTTPClient.STATUS_DISCONNECTED:
		...
		if not _stream_started:
			...
			var err = _client.request(
				HTTPClient.METHOD_POST, "/api/generate", _stream_headers, _stream_body
			)
			if err != OK:
...
func load_config() -> void:
	...
	if not FileAccess.file_exists(path_to_load):
		if FileAccess.file_exists("user://config.json"):
			...
			print(
				"[LLMClient] Config file not found. Using defaults: API URL: ",
				api_url,
				" | DM Model: ",
				world_builder_model,
				" | NPC Model: ",
	...
	if file:
		...
		if json.parse(content) == OK and json.data is Dictionary:
			...
			if "localhost" in api_url:
				...
				print(
					"[LLMClient] Upgraded API URL localhost to 127.0.0.1 to avoid macOS IPv6 handshake latency."
				)
				save_config()
			...
			print(
				"[LLMClient] Config loaded successfully. API URL: ",
				api_url,
				" | DM Model: ",
				world_builder_model,
				" | NPC Model: ",
...
func send_prompt(
	...
	custom_url: String = "",
	stream: bool = true,
	role: String = ROLE_AUTO
) -> void:
	var active_model = model_name if not model_name.is_empty() else character_model
	var active_url = (
		custom_url if not custom_url.is_empty() else api_url.rstrip("/").path_join("api/generate")
	)
	var active_ctx = get_context_length(role, active_model)

	print(
		(
			"[LLMClient] Sending request to model '%s' (Prompt length: %d chars, stream: %s)..."
			% [active_model, prompt.length(), str(stream)]
		)
	)
	...
	var body = {
		...
		"prompt": prompt,
		"stream": stream,
		"options": {"temperature": 0.7, "top_p": 0.9, "num_ctx": active_ctx},
		"keep_alive": -1
	}
...
func test_connection(test_url: String, callback: Callable) -> void:
	...
	temp_http.request_completed.connect(
		func(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray):
			...
			print(
				(
					"[LLMClient] Connection test completed. Result: %d | HTTP Code: %d"
...
func _on_request_completed(
	...
	print(
		"[LLMClient] Response received (raw fallback, Length: %d chars)." % response_text.length()
	)
...
func _enqueue_request(req: Dictionary) -> void:
	...
	if priority == RequestPriority.HIGH:
		var insert_idx = 0
		while (
			insert_idx < _request_queue.size()
			and (
				_request_queue[insert_idx].get("priority", RequestPriority.LOW)
	...
	print(
		(
			"[LLMClient] Enqueued request type '%s' (Priority: %s). Queue size: %d"
			% [req.type, priority_name, _request_queue.size()]
		)
	)
	...
	if priority == RequestPriority.HIGH and _queue_processing and not _active_req.is_empty():
		if _active_req.get("priority", RequestPriority.LOW) == RequestPriority.LOW:
			print(
				(
					"[LLMClient] HIGH priority request enqueued. Cancelling currently running LOW priority request type '%s'..."
					% _active_req.type
				)
			)
...
func _process_queue() -> void:
	...
	print(
		(
			"[LLMClient] Executing queued request type '%s' (Priority: %s). Remaining in queue: %d"
			% [
				req.type,
				(
					"HIGH"
					if req.get("priority", RequestPriority.HIGH) == RequestPriority.HIGH
					else "LOW"
				),
				_request_queue.size()
			]
	...
	match req.type:
		"custom":
			_raw_send_custom_request(
				req.prompt,
				req.model_name,
				req.callback,
				req.timeout,
				req.get("json_mode", false),
				req.get("role", ROLE_AUTO)
			)
		"custom_vision":
			_raw_send_custom_vision_request(
				req.prompt,
				req.image_path,
				req.model_name,
				req.callback,
		...
		"custom_stream":
			_raw_send_custom_stream_request(
				req.prompt,
				req.model_name,
				req.on_chunk,
				req.on_completed,
		...
		"custom_vision_stream":
			_raw_send_custom_vision_stream_request(
				req.prompt,
				req.image_path,
				req.model_name,
				req.on_chunk,
...
func send_custom_vision_request(
	prompt: String,
	image_path: String,
	model_name: String,
	callback: Callable,
...
func send_custom_stream_request(
	prompt: String,
	model_name: String,
	on_chunk: Callable,
...
func send_custom_vision_stream_request(
	prompt: String,
	image_path: String,
	model_name: String,
	on_chunk: Callable,
...
func warmup_model(
	model_name: String, callback: Callable = Callable(), priority: int = RequestPriority.LOW
) -> void:
...
func _raw_send_custom_request(
	...
	print(
		(
			"[LLMClient] _raw_send_custom_request started. URL: %s | Model: %s"
			% [active_url, active_model]
		)
	)
	...
	temp_http.request_completed.connect(
		func(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray):
			...
			print(
				(
					"[LLMClient] _raw_send_custom_request completed. Result: %d | HTTP Code: %d | Body bytes: %d"
			...
			if result == HTTPRequest.RESULT_SUCCESS and response_code == 200:
				...
				error_msg = (
					"HTTP Request failed. Code: " + str(response_code) + " | Result: " + str(result)
				)
	...
	var req_body = {
		...
		"prompt": prompt,
		"stream": false,
		"options": {"temperature": 0.8, "top_p": 0.9, "num_ctx": active_ctx},
		"keep_alive": -1
	}
	if json_mode:
...
func _raw_send_custom_vision_request(
	prompt: String,
	image_path: String,
	model_name: String,
	callback: Callable,
	...
	print(
		(
			"[LLMClient] _raw_send_custom_vision_request started. URL: %s | Model: %s | Image: %s"
			% [active_url, active_model, image_path]
		)
	)
	...
	temp_http.request_completed.connect(
		func(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray):
			...
			print(
				(
					"[LLMClient] _raw_send_custom_vision_request completed. Result: %d | HTTP Code: %d | Body bytes: %d"
					% [result, response_code, body.size()]
				)
			)
			if result == HTTPRequest.RESULT_SUCCESS and response_code == 200:
	...
	var req_body = {
		...
		"prompt": prompt,
		"stream": false,
		"options": {"temperature": 0.5, "top_p": 0.9, "num_ctx": active_ctx},
		"keep_alive": -1
	}
...
func _raw_send_custom_stream_request(
	prompt: String,
	model_name: String,
	on_chunk: Callable,
	on_completed: Callable,
	...
	stream_req.tree_exiting.connect(
		func():
...
func _raw_send_custom_vision_stream_request(
	prompt: String,
	image_path: String,
	model_name: String,
	on_chunk: Callable,
...
func _raw_warmup_model(model_name: String, callback: Callable = Callable()) -> void:
	...
	var body = JSON.stringify({"model": model_name, "prompt": "", "keep_alive": -1})
	var headers = ["Content-Type: application/json"]
	print("[LLMClient] _raw_warmup_model in background: ", model_name)
	temp_http.request_completed.connect(
		func(
			result: int, response_code: int, headers: PackedStringArray, body_bytes: PackedByteArray
		):
			if _active_http_node == temp_http:
				_active_http_node = null
			var success = result == HTTPRequest.RESULT_SUCCESS and response_code == 200
			print(
				"[LLMClient] _raw_warmup_model completed for ",
				model_name,
				". Success: ",
				success,
				" | Result: ",
