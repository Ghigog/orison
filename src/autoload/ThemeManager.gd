	...
	for name in PRESETS.keys():
		...
		if ratio < 4.5:
			printerr(
				...
				name,
				"' fails WCAG AA contrast ratio check: ",
				ratio
			)
...
func save_custom_theme(theme_name: String) -> void:
	...
	if ratio < 4.5:
		print(
			"[ThemeManager] WARNING: Custom theme '",
			theme_name,
			"' fails WCAG AA contrast ratio check: ",
			ratio
		)
...
func apply_theme_to_hierarchy(node: Node) -> void:
	...
	if node is Label:
		...
		if n.contains("emotion"):
			...
			n.contains("desc")
			or n.contains("tagline")
			or n.contains("sub")
			or n.contains("sec")
			or n.contains("reason")
			or n.contains("affinity")
			or n.contains("info")
			or n.contains("status")
			or n.contains("no")
		):
			var sec_color = color_text
			sec_color.a = 0.65
				...
				if sb is StyleBoxFlat:
					...
					if is_accent:
						...
						border_c.a = (
							0.8
							if is_light
							else (sb.border_color.a if sb.border_color.a > 0 else 0.4)
						)
						new_sb.border_color = border_c
