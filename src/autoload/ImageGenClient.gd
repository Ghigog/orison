...
func test_connection(callback: Callable) -> void:
	if mock_handler.is_valid():
		mock_handler.call({"method": "test_connection", "callback": callback})
		return
	...
	temp_http.request_completed.connect(
		func(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray):
			...
			if result == HTTPRequest.RESULT_SUCCESS and response_code == 200:
				...
				error_msg = (
					"Could not connect to local Stable Diffusion API. Make sure it is running on "
					+ LLMClient.image_gen_url
				)
...
func send_txt2img_request(
	prompt: String, width: int = 512, height: int = 512, callback: Callable = Callable()
) -> void:
	if mock_handler.is_valid():
		mock_handler.call(
			{
				...
				if json.parse(text) == OK and json.data is Dictionary:
					...
					if not images_list.is_empty():
						...
						if err == OK:
							...
							error_msg = (
								"Failed to load generated PNG from buffer. Error: " + str(err)
							)
					else:
	...
	var err = temp_http.request(
		target_url, headers, HTTPClient.METHOD_POST, JSON.stringify(req_body)
	)
	if err != OK:
		if callback.is_valid():
...
func send_img2img_request(
	prompt: String, init_image: Image, callback: Callable = Callable()
) -> void:
	if mock_handler.is_valid():
		mock_handler.call(
			{
