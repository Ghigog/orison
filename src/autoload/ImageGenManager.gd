...
const EMOTION_PROMPTS = {
	...
	"serenity":
	"serene expression, calm peaceful face, slight gentle smile, relaxed eyes, tranquil look"
}
...
func generate_asset(
	seed_name: String, prompt: String, category: String, output_path: String
) -> void:
	...
	if CampaignState.campaign_id.is_empty():
		var is_player_avatar = (
			category.to_lower() == "avatar"
			and (
				seed_name == "player"
				or seed_name == "temp_pc_avatar"
				or output_path.get_file().contains("temp_pc_avatar")
				or output_path == "user://temp_pc_avatar.png"
			)
		)
		if not is_player_avatar:
			print(
				"[ImageGenManager] Blocking campaign image generation during onboarding: ",
				output_path
			)
			return
	...
	print(
		(
			"[ImageGenManager] Enqueuing generation for category '%s' with seed '%s'"
			% [category, seed_name]
		)
	)
	...
	_image_request_queue.append(
		{"seed_name": seed_name, "prompt": prompt, "category": category, "output_path": output_path}
	)

	_process_image_queue()


func _raw_generate_asset(
	seed_name: String, prompt: String, category: String, output_path: String
) -> void:
	print(
		(
			"[ImageGenManager] Raw starting generation for category '%s' with seed '%s'"
			% [category, seed_name]
		)
	)
	_is_generating = true
	...
	if LLMClient.image_gen_enabled:
		...
		if CampaignState and CampaignState.state and CampaignState.state.has("adventure_meta"):
			...
			var dropdown = (
				main_root.find_child("CampaignArtStyleDropdown", true, false) as OptionButton
			)
			if dropdown:
		...
		var dispatch_sd = func(final_visual_prompt: String):
			...
			if category.to_lower() == "avatar" or category.to_lower() == "character":
				...
				sd_prompt = (
					"masterpiece, dnd character portrait, detailed facial features, isolated on a solid black background, standalone character sprite, transparent background, png, alpha channel, "
					+ style_mod
					+ ", "
					+ final_visual_prompt
				)
				...
				sd_prompt = (
					"masterpiece, environment scenery concept art, atmospheric background, "
					+ final_visual_prompt
				)
			elif category.to_lower() == "item":
				sd_prompt = (
					"masterpiece, dnd item icon, fantasy object, game icon, glowing details, "
					+ final_visual_prompt
				)

			ImageGenClient.send_txt2img_request(
				sd_prompt,
				width,
				height,
				func(success: bool, refined_image: Image, error_msg: String):
					_generations_in_progress.erase(output_path)
					if success and refined_image:
						...
						if ref_err == OK:
							print(
								"[ImageGenManager] Overwrote placeholder with Stable Diffusion txt2img image: ",
								output_path
							)
		...
		if category.to_lower() == "scene" or category.to_lower() == "location":
			...
			LLMClient.send_custom_request(
				...
				func(llm_success: bool, response_text: String, llm_error: String):
					...
					if llm_success and not response_text.strip_edges().is_empty():
						...
						print(
							"[ImageGenManager] LLM successfully summarized location description to visual prompt: ",
							final_prompt
						)
					else:
						print(
							"[ImageGenManager] LLM summarization failed or returned empty. Falling back to raw description: ",
							llm_error
						)
...
func get_image_or_fallback(
	entity_id: String, entity_type: String, size: Vector2i = Vector2i(0, 0)
) -> Texture2D:
	...
	if FileAccess.file_exists(entity_id):
		...
		match entity_type.to_lower():
			"avatar", "character":
				...
				var emotions_list = [
					"joy", "anger", "sadness", "fear", "trust", "disgust", "surprise", "serenity"
				]
	...
	if entity_type.to_lower() == "item":
		var fallback_image = await procedural_art_engine.generate_item(
			entity_id, entity_id, size.x, size.y
		)
		if fallback_image:
	...
	LLMClient.send_custom_request(
		...
		func(success: bool, response_text: String, error_msg: String):
			...
			if success and not response_text.strip_edges().is_empty():
				...
				print(
					(
						"[ImageGenManager] LLM portrait prompt for '%s': %s"
						% [char_name, visual_prompt]
					)
				)
			else:
				print(
					(
						"[ImageGenManager] LLM portrait prompt failed (%s), using name fallback."
						% error_msg
					)
				)
...
func get_avatar_path(char_id: String, emotion: String = "") -> String:
	...
	return (
		"user://adventures/%s/generated_assets/character_%s_%s.png"
		% [campaign_id, char_id, emotion]
	)
