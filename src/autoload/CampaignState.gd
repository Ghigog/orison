...
func initialize(id: String, save_data: Dictionary) -> void:
	apply_state_change({"type": "initialize", "id": id, "save_data": save_data})
	...
	match type:
		"initialize":
			...
			print(
				...
				title,
				"' save structure initialized (Playtime: ",
				playtime_seconds,
				"s)."
			)
			...
			if not graph_manager.has_node(char_id):
				...
				if not base_emotion.is_empty():
					...
					if not valid_emotions.has(emotion):
						emotion = "serenity"
					_add_emotion_event_internal(
						char_id,
						emotion,
						base_intensity if base_intensity >= 0.0 else 0.5,
						"player",
						"Initial state."
					)
			else:
...
func _add_emotion_event_internal(
	char_id: String,
	emotion: String,
	intensity: float,
	target: String,
...
func save() -> Error:
	...
	state["metadata"]["engine_version"] = ProjectSettings.get_setting(
...
func set_campaign_meta(key: String, value: Variant) -> void:
	apply_state_change({"type": "set_campaign_meta", "key": key, "value": value})
...
func set_plot_state(key: String, value: Variant) -> void:
	apply_state_change({"type": "set_plot_state", "key": key, "value": value})
...
func adjust_affinity(char_id: String, delta: float) -> float:
	return apply_state_change({"type": "adjust_affinity", "char_id": char_id, "delta": delta})


func add_emotion_event(
	char_id: String,
	emotion: String,
	intensity: float,
	target: String,
...
func add_to_inventory(
	char_id: String, item_name: String, quantity: int, properties: Dictionary = {}
) -> void:
	apply_state_change(
		{
...
func add_history_log(
	role: String, content: String, sender: String = "", active_char: String = ""
) -> void:
	apply_state_change(
		{
...
func log_state_summary() -> void:
	...
	_mutex.unlock()
	print(
		"[CampaignState] Campaign state active: '",
		title,
		"'. Loaded setup details: Characters: ",
		char_count,
		" | Graph Nodes: ",
		node_count,
		" | Graph Edges: ",
...
func set_player_character(pc: Dictionary) -> void:
	apply_state_change({"type": "set_player_character", "pc": pc})


func set_memory(memory: Dictionary) -> void:
	apply_state_change({"type": "set_memory", "memory": memory})


func set_pending_scene(scene: Dictionary) -> void:
	apply_state_change({"type": "set_pending_scene", "scene": scene})


func set_turns_since_last_director(turns: int) -> void:
	apply_state_change({"type": "set_turns_since_last_director", "turns": turns})


func set_director_cooldown(cooldown: int) -> void:
	apply_state_change({"type": "set_director_cooldown", "cooldown": cooldown})


func set_history_logs(logs: Array) -> void:
	apply_state_change({"type": "set_history_logs", "logs": logs})


func set_knowledge_graph_data(nodes: Dictionary, edges: Array) -> void:
	apply_state_change({"type": "set_knowledge_graph_data", "nodes": nodes, "edges": edges})


func update_character_properties(char_id: String, properties: Dictionary) -> void:
	apply_state_change(
		{"type": "update_character_properties", "char_id": char_id, "properties": properties}
	)
...
func set_last_director_beat(beat: String) -> void:
	apply_state_change({"type": "set_last_director_beat", "beat": beat})
...
func _on_location_changed(new_location: String) -> void:
	...
	if new_location != _last_autosave_location:
		_last_autosave_location = new_location
		print(
			"[CampaignState] Location changed from old to '",
			new_location,
			"'. Triggering autosave."
		)
		trigger_autosave()
