...
func test_json_save_and_state() -> bool:
	...
	CampaignState.init_character(
		"char_test", "Test NPC", "A script testing npc.", "A tests writing style.", "", "anger", 0.7
	)
	...
	if (
		...
		or CampaignState.get_character("char_test").get("writing_style") != "A tests writing style."
	):
		return false
...
func test_system_prompts() -> bool:
	...
	CampaignState.init_character(
		"elara", "Elara the Wise", "A friendly wizard.", "Speaks formally."
	)
...
func test_image_generation_pipeline() -> bool:
	...
	if not item_custom or item_custom.get_width() != 512 or item_custom.get_height() != 512:
		print(
			"[FAIL] Procedural item custom 512x512 dimensions are invalid: ", item_custom.get_size()
		)
		engine.queue_free()
		return false
...
func test_emotion_engine_and_decay() -> bool:
	...
	CampaignState.init_character(
		"test_npc", "Test NPC", "A biography.", "", "", "serenity", 0.5, 0.0
	)
	...
	CampaignState.init_character(
		"reinforced_npc", "Reinforced NPC", "A biography.", "", "", "anger", 0.8, 0.0
	)
...
func test_atomic_writes_and_upgrade() -> bool:
	...
	var legacy_data = {"adventure_meta": {"campaign_id": campaign_id, "title": "Legacy Campaign"}}
...
func test_emotion_engine_calculations() -> bool:
	...
	var invalid_tags = {
		"emotion": "angry", "intensity": 0.5, "target": "player", "reason": "Test invalid emotion."
	}
	...
	var delta_skip_tags = {
		"emotion": "serenity", "intensity": 0.5, "target": "player", "reason": "Identical emotion"
	}
	...
	var delta_skip_tags_close = {
		"emotion": "serenity", "intensity": 0.53, "target": "player", "reason": "Very close emotion"
	}
...
func test_llm_stream_parsing() -> bool:
	...
	if parser_state["zones_ended_count"] != 1:
		print(
			"[FAIL] LLMStreamParser zone_ended trigger failed: ", parser_state["zones_ended_count"]
		)
		return false
	...
	if parser_state["zones_started"].size() != 1 or parser_state["zones_started"][0] != "dialogue":
		print(
			"[FAIL] LLMStreamParser dialogue zone_started failed: ", parser_state["zones_started"]
		)
		return false
	if parser_state["chars_received"] != "Watch out!":
		print(
			"[FAIL] LLMStreamParser dialogue char_received failed: ", parser_state["chars_received"]
		)
		return false
...
func test_memory_summarization_pipeline() -> bool:
	...
	if mt_memories.size() != 0:
		print(
			"[FAIL] Distilled medium term memories did not clear, remaining: ", mt_memories.size()
		)
		LLMClient.mock_response_handler = Callable()
		dummy_controller.free()
		return false
...
func test_character_creator_validation() -> bool:
	...
	creator_node.setup_character(
		{"name": "", "physical_description": "", "personality": "", "backstory": "", "avatar": ""}
	)
	...
	if creator_node.pc_backstory_input.text.length() != 2000:
		print(
			"[FAIL] Text capping failed. Length is: ", creator_node.pc_backstory_input.text.length()
		)
		remove_child(creator_node)
		creator_node.free()
		return false
...
func test_campaign_graph_view_filtering() -> bool:
	...
	if not is_equal_approx(tavern_node.modulate.a, 0.25):
		print(
			"[FAIL] Non-matching node should be dimmed (alpha=0.25). Got: ", tavern_node.modulate.a
		)
		remove_child(view)
		view.free()
		return false
...
func test_transparent_avatars_and_emotions() -> bool:
	...
	CampaignState.init_character(
		"test_character", "Test Character", "A character with gold wings.", "", img_path
	)
