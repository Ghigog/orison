...
func _ready() -> void:
	...
	var test_descriptions = {
		...
		"test_campaign_state_mutex_and_autosave":
		"CampaignState Mutex, Playtime & Autosave Operations",
		"test_media_manager_and_copying": "MediaManager Operations, Copying & Generator Hooks",
		"test_location_description_extraction":
		"Location Description Extraction & LLM Summarization",
		"test_draggable_chat_box": "Draggable & Floating Chat Window with Side Toolbar",
		"test_llm_stream_request_timeout_prevented_by_chunks":
		"LLMStreamRequest Active Stream Timeout Prevention",
		"test_gender_fallbacks_and_persistence":
		"Gender Fallbacks, Inference Prompt Rules & KG Persistence",
		"test_director_threshold_and_narration_beat":
		"Director Threshold & Narration Beat Injection",
...
func test_json_save_and_state() -> bool:
	...
	while true:
		...
		CampaignState.init_character(
			...
			"Test NPC",
			{
				"biography": "A script testing npc.",
				"writing_style": "A tests writing style.",
				"base_emotion": "anger",
				"base_intensity": 0.7
			}
		)
		...
		if (
			char_data.get("name") != "Test NPC"
			or char_data.get("writing_style") != "A tests writing style."
		):
			passed = false
			break
		...
		if (
			initial_emotions.is_empty()
			or initial_emotions[0].get("emotion") != "anger"
			or initial_emotions[0].get("intensity") != 0.7
		):
			passed = false
			break
		...
		# 4. Emotion event
		CampaignState.add_emotion_event(
			"char_test", "joy", 0.9, "player", "Translating ancient texts."
		)
		...
		if (
			CampaignState.get_character("char_test").get("affinity") != 0.25
			or (
				CampaignState.get_character("char_test").get("writing_style")
				!= "A tests writing style."
			)
		):
			passed = false
			break
...
"""
		...
		if (
			fm.get("type") != "character"
			or fm.get("name") != "Elara the Wise"
			or fm.get("writing_style") != "Speaks with wisdom."
		):
			passed = false
			break
		...
		if tables.size() != 1:
			...
			break
		if (
			tables[0]["headers"].size() != 2
			or tables[0]["headers"][0] != "Item"
			or tables[0]["headers"][1] != "Value"
		):
			passed = false
			break
...
"""
		...
		if (
			character_compiled.get("base_emotion") != "anger"
			or character_compiled.get("base_intensity") != 0.85
		):
			passed = false
			break

		if (
			character_compiled.get("is_creature") != false
			or character_compiled.get("can_speak") != true
			or character_compiled.get("humanoid") != true
		):
			passed = false
			break
		...
		if (
			creature_compiled.get("is_creature") != true
			or creature_compiled.get("can_speak") != false
			or creature_compiled.get("humanoid") != false
		):
			passed = false
			break
		...
		for edge in edges:
			if (
				edge.from == "test_character"
				and edge.to == "fauna_skiver"
...
func test_system_prompts() -> bool:
	...
	while true:
		...
		if wb_prompt.is_empty():
			...
			break
		if (
			not wb_prompt.contains("WORLD BUILDER")
			or not wb_prompt.contains("narration")
			or not wb_prompt.contains("choices")
		):
			passed = false
			break
		...
		if ca_prompt.is_empty():
			...
			break
		if (
			not ca_prompt.contains("Thorin")
			or not ca_prompt.contains("blacksmith")
			or not ca_prompt.contains("Enemy")
			or not ca_prompt.contains("Anger")
			or not ca_prompt.contains("Speaks in dwarf slang.")
		):
			passed = false
			break
		...
		CampaignState.init_character(
			"elara",
			"Elara the Wise",
			{"biography": "A friendly wizard.", "writing_style": "Speaks formally."}
		)
		...
		if id_prompt.is_empty():
			...
			break
		if (
			not id_prompt.contains("Elara the Wise")
			or not id_prompt.contains("Friend")
			or not id_prompt.contains("Joy")
			or not id_prompt.contains("shared a map")
			or not id_prompt.contains("Speaks formally.")
		):
			passed = false
			break
		...
		CampaignState.init_character("valen", "Valen the Ranger", {"biography": "A ranger."})
		...
		if dm_test_prompt.is_empty():
			...
			break
		if (
			not dm_test_prompt.contains("memory_updates")
			or not dm_test_prompt.contains("Adventure Memories")
			or not dm_test_prompt.contains("Player Action/Input")
		):
			passed = false
			break

		# 6. Verify Emotion Reflection Prompt
		var refl_prompt = PromptBuilder.get_emotion_reflection_prompt_for_id(
			"valen", "A sudden bolt of lightning strikes the tower nearby."
		)
		if refl_prompt.is_empty():
			passed = false
			break
		if (
			not refl_prompt.contains("Valen the Ranger")
			or not refl_prompt.contains("ENVIRONMENTAL EVENT")
			or not refl_prompt.contains("lightning strikes")
			or not refl_prompt.contains("emotional_update")
		):
			passed = false
			break

		# 7. Verify Base Emotion Deduction Prompt
		var deduce_prompt = SystemPrompts.get_deduce_base_emotion_prompt(
			"Thorin", "A grumpy dwarf blacksmith who is always angry at his apprentice."
		)
		if deduce_prompt.is_empty():
			passed = false
			break
		if (
			not deduce_prompt.contains("Thorin")
			or not deduce_prompt.contains("grumpy dwarf")
			or not deduce_prompt.contains("base_emotion")
			or not deduce_prompt.contains("base_intensity")
		):
			passed = false
			break
		...
		var sel_prompt = SystemPrompts.get_starters_selection_prompt(
			"Witcher Campaign", test_clusters, pc_test
		)
		if sel_prompt.is_empty():
			passed = false
			break
		if (
			not sel_prompt.contains("Witcher Campaign")
			or not sel_prompt.contains("amber_outpost")
			or not sel_prompt.contains("sita")
			or not sel_prompt.contains("JSON RESPONSE SCHEMA")
		):
			passed = false
			break
		...
		if narr_prompt.is_empty():
			...
			break
		if (
			not narr_prompt.contains("The Shattered Vigil")
			or not narr_prompt.contains("Sita")
			or not narr_prompt.contains("Amber Outpost")
			or not narr_prompt.contains("Grimdark tone.")
		):
			passed = false
			break
...
func test_beginning_prompt() -> bool:
	...
	if prompt.is_empty():
		return false
	if (
		not prompt.contains("Lost Crypt Campaign")
		or not prompt.contains("Entrance to Crypt")
		or not prompt.contains("Descriptive, epic fantasy style.")
	):
		return false
...
"""
		...
		if (
			scanned.potential_characters.is_empty()
			or scanned.potential_characters[0].id != "sita_katchent"
		):
			passed = false
			break
		...
		if compiled.characters.is_empty() or not compiled.characters.has("sita_katchent"):
			...
			break
		if (
			not compiled.knowledge_graph.nodes.has("yild")
			or compiled.knowledge_graph.nodes["yild"].get("type") != "location"
		):
			passed = false
			break
...
func test_theme_manager_and_font_scaling() -> bool:
	...
	while true:
		...
		if (
			ThemeManager.spacing_xs != 4.0
			or ThemeManager.spacing_sm != 8.0
			or ThemeManager.spacing_md != 16.0
			or ThemeManager.spacing_lg != 24.0
			or ThemeManager.spacing_xl != 40.0
		):
		...
		if (
			ThemeManager.radius_sm != 0.0
			or ThemeManager.radius_md != 0.0
			or ThemeManager.radius_lg != 0.0
			or ThemeManager.radius_pill != 0.0
		):
		...
		if (
			not ThemeManager.shadow_subtle
			or not ThemeManager.shadow_medium
			or not ThemeManager.shadow_elevated
		):
		...
		if (
			ThemeManager.duration_fast != 0.12
			or ThemeManager.duration_normal != 0.25
			or ThemeManager.duration_slow != 0.4
		):
		...
		if (
			ThemeManager.color_success != Color("#10B981")
			or ThemeManager.color_danger != Color("#E11D48")
		):
		...
		if (
			ThemeManager.get_emotion_color("joy") != Color("#F59E0B")
			and ThemeManager.get_emotion_color("joy") != Color("#D97706")
		):
...
func test_player_input_parser() -> bool:
	...
	while true:
		...
		if (
			res1.dialogue != "Take a look at that"
			or res1.action != "I shout"
			or res1.detected_format != "quotes"
		):
		...
		if (
			res1b.dialogue != "Wait, look at this."
			or res1b.action != "I said"
			or res1b.detected_format != "quotes"
		):
		...
		if (
			res2.dialogue != "Take a look at that"
			or res2.action != "shouting"
			or res2.detected_format != "asterisks"
		):
		...
		if (
			res3.dialogue != ""
			or res3.action != "I shout at andino to look out for that rock."
			or res3.detected_format != "pure_action"
		):
		...
		if (
			res3b.dialogue != ""
			or res3b.action != "We search the ruins carefully."
			or res3b.detected_format != "pure_action"
		):
...
func test_image_generation_pipeline() -> bool:
	...
	while true:
		...
		var avatar_img = await engine.generate_avatar(
			"Player", "A brave warrior with red hair and brown leather armor."
		)
		...
		var avatar_custom = await engine.generate_avatar(
			"Player", "A brave warrior with red hair", 512, 512
		)
		if (
			not avatar_custom
			or avatar_custom.get_width() != 512
			or avatar_custom.get_height() != 512
		):
			print(
				"[FAIL] Procedural avatar custom 512x512 dimensions are invalid: ",
				avatar_custom.get_size()
			)
			engine.queue_free()
			passed = false
			break
		...
		var scene_img = await engine.generate_scene(
			"A sunny meadow with green grass and a blue sky."
		)
		...
		if not scene_custom or scene_custom.get_width() != 768 or scene_custom.get_height() != 512:
			print(
				"[FAIL] Procedural scene custom 768x512 dimensions are invalid: ",
				scene_custom.get_size()
			)
			engine.queue_free()
			passed = false
			break
		...
		var item_custom = await engine.generate_item(
			"Sword of Light", "A glowing silver sword", 512, 512
		)
		if not item_custom or item_custom.get_width() != 512 or item_custom.get_height() != 512:
			print(
				"[FAIL] Procedural item custom 512x512 dimensions are invalid: ",
				item_custom.get_size()
			)
			engine.queue_free()
			passed = false
			break
		...
		if err != OK:
			print(
				"[FAIL] Base64 decoding failed to restore valid PNG image structure. Error: ", err
			)
			passed = false
			break
...
func test_llm_request_queue() -> bool:
	...
	if (
		LLMClient._request_queue[0].prompt != "Prompt 2"
		or LLMClient._request_queue[1].prompt != "Prompt 3"
	):
...
func test_prompt_budgeting() -> bool:
	...
	while true:
		...
		var compressed1 = PromptBuilder.compress_history_content(
			"(smiling and waving) Hello there friend!"
		)
		...
		for role in [LLMClient.ROLE_CHARACTER, LLMClient.ROLE_WORLD_BUILDER]:
			...
			if budget >= ctx:
				print(
					(
						"[FAIL] Prompt budget %d for role '%s' leaves no room for the response (context %d)"
						% [budget, role, ctx]
					)
				)
				passed = false
			...
			if ctx - budget != LLMClient.PROMPT_RESPONSE_RESERVE:
				print(
					(
						"[FAIL] Response reserve for role '%s' is %d, expected %d"
						% [role, ctx - budget, LLMClient.PROMPT_RESPONSE_RESERVE]
					)
				)
				passed = false
				break
		...
		if LLMClient.get_context_length(LLMClient.ROLE_WORLD_BUILDER) != 8192:
			print(
				(
					"[FAIL] World builder context resolved to %d with a shared model name"
					% LLMClient.get_context_length(LLMClient.ROLE_WORLD_BUILDER)
				)
			)
		...
		if LLMClient.get_context_length(LLMClient.ROLE_CHARACTER) != 4096:
			print(
				(
					"[FAIL] Character context resolved to %d with a shared model name"
					% LLMClient.get_context_length(LLMClient.ROLE_CHARACTER)
				)
			)
			LLMClient.world_builder_context = saved_wb_ctx
			LLMClient.character_context = saved_char_ctx
		...
		CampaignState.set_knowledge_graph_data(
			{
				"castle":
				{
					"label": "Castle",
					"type": "Location",
					"desc": "A very large fortified castle built of ancient grey stone."
				},
				"king":
				{"label": "King", "type": "Character", "desc": "The elderly king of the realm."}
			},
		...
		if "Castle" in budgeted_ctx or "resides_at" in budgeted_ctx:
			print(
				"[FAIL] Budgeted lore context failed to truncate distant connections: ",
				budgeted_ctx
			)
			passed = false
			break
...
func test_emotion_engine_and_decay() -> bool:
	...
	while true:
		...
		CampaignState.add_emotion_event(
			"reinforced_npc", "anger", 0.8, "player", "Setup reinforced.", 0.0
		)
		...
		if test_npc_last.get("emotion") != "serenity":
			print(
				"[FAIL] Decay failed to change emotion to serenity at 0.0 baseline: ",
				test_npc_last.get("emotion")
			)
			passed = false
			break
...
func test_split_configs() -> bool:
	...
	if (
		LLMClient.api_url != "http://127.0.0.1:9999"
		or LLMClient.world_builder_model != "test-gemma"
		or LLMClient.character_model != "test-llama"
	):
...
func test_atomic_writes_and_upgrade() -> bool:
	...
	while true:
		...
		var legacy_data = {
			"adventure_meta": {"campaign_id": campaign_id, "title": "Legacy Campaign"}
		}
		...
		if not file:
			print(
				(
					"[FAIL] Could not open legacy fixture for writing at %s (error %d)"
					% [final_path, FileAccess.get_open_error()]
				)
			)
			passed = false
			break
		...
		if (
			not loaded.has("plot_states")
			or not loaded.has("knowledge_graph")
			or loaded.has("characters")
			or not loaded.has("history_logs")
		):
...
func test_texture_caching() -> bool:
	...
	CampaignState.init_character("cache_npc", "Cache NPC", {"biography": "Bio", "avatar": img_path})
...
func test_event_bus_signals() -> bool:
	...
	while true:
		...
		if not signals_fired["prompt"]:
			print(
				"[FAIL] EventBus director_prompt_generated signal did not fire or parameter mismatched"
			)
			passed = false
			break
		if not signals_fired["char"]:
			print(
				"[FAIL] EventBus character_state_updated signal did not fire or parameter mismatched"
			)
			passed = false
			break
...
func test_emotion_engine_calculations() -> bool:
	...
	while true:
		...
		CampaignState.init_character(
			"test_npc",
			"Test NPC",
			{"biography": "Bio", "base_emotion": "serenity", "base_intensity": 0.5, "affinity": 0.0}
		)
		...
		if (
			emotions.is_empty()
			or emotions[-1].get("emotion") != "joy"
			or not is_equal_approx(emotions[-1].get("intensity"), 0.8)
		):
		...
		if (
			not is_equal_approx(emotions[-1].get("intensity"), 1.0)
			or not is_equal_approx(emotions[-1].get("rapport_delta"), 0.2)
		):
		...
		if not is_equal_approx(char_data.get("affinity"), 0.3):  # 0.1 + 0.2 (clamped delta)
			print(
				"[FAIL] EmotionEngine high boundary affinity accumulation failed: ",
				char_data.get("affinity")
			)
			passed = false
			break
		...
		emotions = char_data.get("emotions", [])
		if (
			not is_equal_approx(emotions[-1].get("intensity"), 0.0)
			or not is_equal_approx(emotions[-1].get("rapport_delta"), -0.2)
		):
		...
		if not is_equal_approx(char_data.get("affinity"), 0.1):  # 0.3 - 0.2 (clamped delta)
			print(
				"[FAIL] EmotionEngine low boundary affinity accumulation failed: ",
				char_data.get("affinity")
			)
			passed = false
			break
		...
		if emotions[-1].get("emotion") != "serenity":
			print(
				"[FAIL] EmotionEngine invalid emotion fallback failed: ",
		...
		var record_emit = func(char_id, emotion, affinity): signal_state["emitted"] = true
		...
		if signal_state["emitted"]:
			print(
				"[FAIL] EmotionEngine emitted character_visual_update_requested for identical emotion (RAG006 failed)"
			)
			passed = false
			break
		...
		if signal_state["emitted"]:
			print(
				"[FAIL] EmotionEngine emitted character_visual_update_requested for close intensity change (<0.05) (RAG006 failed)"
			)
			passed = false
			break
		...
		if not signal_state["emitted"]:
			print(
				"[FAIL] EmotionEngine did not emit character_visual_update_requested for larger intensity change (RAG006 failed)"
			)
			passed = false
			break
...
func test_json_repair_edge_cases() -> bool:
	...
	if parsed.get("response") != "Hello single quotes.":
		...
		return false
	if (
		parsed.get("emotional_update", {}).get("emotion") != "joy"
		or not is_equal_approx(parsed.get("emotional_update", {}).get("intensity"), 0.7)
	):
	...
	if (
		parsed.get("response") != "Hello"
		or not parsed.get("items") is Array
		or parsed.get("items").size() != 2
	):
...
func test_llm_stream_parsing() -> bool:
	...
	while true:
		...
		var chunks = []
		var state = {"is_completed": false, "completed_text": ""}
		...
		req.start(
			...
			"model",
			2048,
			func(word: String): chunks.append(word),
			func(text: String):
		...
		if not state["is_completed"] or state["completed_text"] != "Hello world!":
			print(
				"[FAIL] LLMStreamRequest completion callback failed: is_completed=",
				state["is_completed"],
				" completed_text=",
		...
		parser.reset()
		var parser_state = {"zones_started": [], "chars_received": "", "zones_ended_count": 0}

		parser.zone_started.connect(
			func(zone_name): parser_state["zones_started"].append(zone_name)
		)
		...
		if (
			parser_state["zones_started"].size() != 1
			or parser_state["zones_started"][0] != "narration"
		):
			print(
				"[FAIL] LLMStreamParser zone_started trigger failed: ",
				parser_state["zones_started"]
			)
			passed = false
			break
		if parser_state["chars_received"] != "The rain fell heavily on the roof.":
			print(
				"[FAIL] LLMStreamParser char_received accumulation failed: ",
		...
		if parser_state["zones_ended_count"] != 1:
			print(
				"[FAIL] LLMStreamParser zone_ended trigger failed: ",
				parser_state["zones_ended_count"]
			)
			passed = false
			break
		...
		if (
			parser_state["zones_started"].size() != 1
			or parser_state["zones_started"][0] != "dialogue"
		):
			print(
				"[FAIL] LLMStreamParser dialogue zone_started failed: ",
				parser_state["zones_started"]
			)
			passed = false
			break
		if parser_state["chars_received"] != "Watch out!":
			print(
				"[FAIL] LLMStreamParser dialogue char_received failed: ",
				parser_state["chars_received"]
			)
			passed = false
			break
...
"""
		...
		if wiki_links.size() != 2:
			...
			break
		if (
			wiki_links[0]["target"] != "target_without_label"
			or wiki_links[0]["label"] != "target_without_label"
		):
			print(
				"[FAIL] MarkdownParser edge cases: wiki-link without label failed: ", wiki_links[0]
			)
			passed = false
			break
		if (
			wiki_links[1]["target"] != "target_with_label"
			or wiki_links[1]["label"] != "Custom Label"
		):
		...
		if callouts[0]["type"] != "secret" or callouts[0]["title"] != "Inside Callout":
			...
			break
		if (
			not callouts[0]["content"].contains("Hidden gold coins!")
			or not callouts[0]["content"].contains("Under the floorboards.")
		):
...
func test_knowledge_graph_traversal() -> bool:
	...
	if not member_ids.has("char_elara") or not member_ids.has("char_thorin"):
		print(
			"[FAIL] KnowledgeGraph Traversal: members did not include Elara or Thorin: ", member_ids
		)
		return false
	...
	if not connection_ids.has("loc_phandalin"):
		print(
			"[FAIL] KnowledgeGraph Traversal: Elara to Phandalin multi-hop failed: ", connection_ids
		)
		return false
...
func test_save_migration_knowledge_graph() -> bool:
	var legacy_save = {
		...
		"adventure_meta":
		{"campaign_id": "migration_test", "title": "Migration Campaign", "version": "0.1.0"},
		"characters":
		{
			"npc_marcello":
			{
	...
	if (
		node.get("type") != "character"
		or node.get("label") != "Marcello"
		or node.get("desc") != "A wealthy merchant."
	):
...
func test_memory_summarization_pipeline() -> bool:
	...
	while true:
		...
		CampaignState.init_character(char_id, "NPC", {"biography": "Biography"})

		# Verify lazy properties initialization
		var character = CampaignState.get_character(char_id)
		if (
			not character.has("medium_term_memories")
			or not character.has("long_term_memory")
			or not character.has("turns_since_last_summary")
		):
		...
		LLMClient.mock_response_handler = func(
			prompt: String, model_name: String, callback: Callable, timeout: float
		):
			mock_call_count[0] += 1
		...
		if remaining_history.size() != 10:
			print(
				"[FAIL] Compaction did not reduce history size correctly. Remaining: ",
		...
		if mt_memories.size() != 0:
			print(
				"[FAIL] Distilled medium term memories did not clear, remaining: ",
				mt_memories.size()
			)
			LLMClient.mock_response_handler = Callable()
			dummy_controller.free()
...
func test_character_creator_validation() -> bool:
	...
	while true:
		...
		if creator_node.pc_backstory_input.text.length() != 2000:
			print(
				"[FAIL] Text capping failed. Length is: ",
				creator_node.pc_backstory_input.text.length()
			)
			remove_child(creator_node)
			creator_node.free()
			passed = false
...
func test_chat_history_virtualization() -> bool:
		...
		test_messages.append(
			{
				...
				"text":
				(
					"This is message number %d. It contains some text that wraps. " % i
					+ "A".repeat(i % 100)
				),
				"type": type,
				"is_temporary": false
	...
	if scroll_container._total_height <= 0.0:
		print(
			"[FAIL] VirtualScrollContainer total height should be positive after setting messages."
		)
	...
	if active_count_initial == 0 or active_count_initial >= test_messages.size():
		print(
			"[FAIL] VirtualScrollContainer should only instantiate visible nodes. Active count: ",
			active_count_initial
		)
		remove_child(scroll_container)
		scroll_container.free()
		return false
...
func test_procedural_image_fallback() -> bool:
	...
	CampaignState.init_character("no_avatar_npc", "Sir Reginald", {"biography": "A noble knight."})
	...
	if state_disabled.status != "error":
		print(
			"[FAIL] Asset status should be 'error' when image gen is disabled but got: ",
			state_disabled.status
		)
		LLMClient.image_gen_enabled = old_enabled
		SaveManager.delete_campaign(campaign_id)
		return false
	...
	while true:
		...
		var mock_data = {
			"nodes":
			{
				...
				"lore_book":
				{"label": "Ancient Book", "type": "lore", "desc": "A dust-covered book."},
				"scene_intro":
				{"label": "Intro Scene", "type": "scene", "desc": "The story begins here."}
			},
			"edges":
			[
		...
		if (
			not view.graph_edit.has_node("loc_tavern")
			or not view.graph_edit.has_node("char_elara")
			or not view.graph_edit.has_node("lore_book")
			or not view.graph_edit.has_node("scene_intro")
		):
		...
		if not "Showing 3 of 4" in view.node_count_label.text:
			print(
				"[FAIL] Count label did not update correctly on category disable. Got: ",
				view.node_count_label.text
			)
			remove_child(view)
			view.free()
			passed = false
		...
		if not is_equal_approx(tavern_node.modulate.a, 0.25):
			print(
				"[FAIL] Non-matching node should be dimmed (alpha=0.25). Got: ",
				tavern_node.modulate.a
			)
			remove_child(view)
			view.free()
			passed = false
		...
		if not "Showing 1 of 4" in view.node_count_label.text:
			print(
				"[FAIL] Count label did not update correctly on search query. Got: ",
				view.node_count_label.text
			)
			remove_child(view)
			view.free()
		...
		if not is_equal_approx(view.graph_edit.zoom, 1.0):
			print(
				"[FAIL] Zoom Reset button did not restore zoom to 1.0. Got: ", view.graph_edit.zoom
			)
			remove_child(view)
			view.free()
			passed = false
...
func test_json_repair_diagnostics() -> bool:
	...
	if not parsed.has("error") or parsed.get("error") == "":
		...
		return false
	if (
		not "Missing opening bracket" in parsed.get("error")
		and not "Parse error" in parsed.get("error")
	):
...
func test_llm_stream_request_timeout() -> bool:
	...
	var req = LLMStreamRequest.new()
	var state = {"failed_called": false, "error_received": ""}
...
func test_llm_stream_request_timeout_prevented_by_chunks() -> bool:
	...
	if "Request timed out" in state["error_received"]:
		print(
			"[FAIL] LLMStreamRequest timed out even though chunks were received: ",
...
func test_vault_scanner_progress() -> bool:
	...
	var on_progress = func(current: int, total: int): progress_signals.append([current, total])
	...
	if last_signal[0] != last_signal[1] or last_signal[1] != 2:
		print(
			"[FAIL] VaultScanner progress did not reach total of 2 files. Got: ", progress_signals
		)
		return false
...
func test_theme_contrast_validation() -> bool:
	...
	var low_contrast_theme = {"color_bg": "#FFFFFF", "color_text": "#FDFDFD"}
...
func test_campaign_state_mutex_and_autosave() -> bool:
	...
	while true:
		...
		if CampaignState.playtime_seconds != 1.5:
			print(
				"[FAIL] Playtime did not increment correctly. Got: ", CampaignState.playtime_seconds
			)
			passed = false
			break
...
func test_media_manager_and_copying() -> bool:
	...
	while true:
		...
		(
			write_test_file
			. call(
				vault_path.path_join("crypt_of_shadows.md"),
				"""---
type: location
...
"""
		...
		print(
			"[DEBUG] test_media_manager_and_copying compilation finished, compiled nodes: ",
			compiled.get("knowledge_graph", {}).get("nodes", {}).keys()
		)
		...
		CampaignState.initialize(
			"test_media_campaign", SaveManager.create_campaign("test_media_campaign", "Test BGM")
		)
		CampaignState.set_knowledge_graph_data(
			nodes, compiled.get("knowledge_graph", {}).get("edges", [])
		)
		...
		test_media_manager.register_tts_generator(
			func(text, voice_id, path):
				tts_state["called"] = true
				tts_state["text"] = text
		)

		test_media_manager.generate_tts(
			"Test Speech Output", "elara_voice", "user://test_tts_out.wav"
		)
...
"""
	...
	LLMClient.mock_response_handler = func(
		prompt_text: String, model_name: String, callback: Callable, timeout: float
	):
...
func test_transparent_avatars_and_emotions() -> bool:
	...
	while true:
		...
		CampaignState.initialize(
			campaign_id, SaveManager.create_campaign(campaign_id, "Test Emo Campaign")
		)
		...
		CampaignState.init_character(
			"test_character",
			"Test Character",
			{"biography": "A character with gold wings.", "avatar": img_path}
		)
		...
		DirAccess.remove_absolute(img_path)
		var target_path = (
			"user://adventures/%s/generated_assets/character_test_character_anger.png" % campaign_id
		)
		if FileAccess.file_exists(target_path):
			DirAccess.remove_absolute(target_path)
...
func test_draggable_chat_box() -> bool:
	...
	if LLMClient.chat_box_position_x != 425.0 or LLMClient.chat_box_position_y != 120.0:
		print(
			(
				"[FAIL] LLMClient did not restore chat box positions. Found x=%f, y=%f"
				% [LLMClient.chat_box_position_x, LLMClient.chat_box_position_y]
			)
		)
...
func test_gender_fallbacks_and_persistence() -> bool:
	...
	while true:
		...
		if not expected_fallback_rule in prompt:
			print(
				"[FAIL] SystemPrompts character agent prompt does not contain correct gender inference fallback instruction."
			)
			passed = false
			break
		...
		CampaignState.initialize(
			test_save_id, SaveManager.create_campaign(test_save_id, "Test Gender Campaign")
		)
		...
		for compiled_node_id in graph.nodes.keys():
			...
			CampaignState.graph_manager.add_node(
				compiled_node_id, n.label, n.type, n.desc, n.properties
			)
		...
		if compiled_gender != "male, he/him":
			print(
				(
					"[FAIL] Frontmatter he/him: true fallback did not compile to 'male, he/him'. Found: '%s'"
					% compiled_gender
				)
			)
			passed = false
		...
		if loaded_gender != "male, he/him":
			print(
				(
					"[FAIL] Loaded character gender does not match compiled gender. Found: '%s'"
					% loaded_gender
				)
			)
			passed = false
...
func test_director_threshold_and_narration_beat() -> bool:
	...
	while true:
		...
		CampaignState.initialize(
			test_save_id, SaveManager.create_campaign(test_save_id, "Test Director Beat Campaign")
		)
		...
		if CampaignState.get_last_director_beat() != "A mysterious shadow looms over the valley.":
			print(
				(
					"[FAIL] last_director_beat did not persist in save file. Found: '%s'"
					% CampaignState.get_last_director_beat()
				)
			)
			passed = false
			break
		...
		CampaignState.init_character(
			"test_beat_npc", "Test NPC", {"biography": "A friendly tester."}
		)
...
"""
		...
		# Run VaultCompiler
		var compiled = await VaultCompiler.compile_vault(
			test_vault, {"campaign_id": "test_raptor_campaign"}
		)
		...
		LLMClient.mock_embedding_handler = func(text: String) -> Array: return [0.0, 0.0, 1.0]  # match L2
...
func test_react_loop() -> bool:
	...
	CampaignState.state = {
		...
		"adventure_meta": {"title": "React Test Campaign"},
		"campaign_meta": {"active_location": "the_swamp", "active_character": "aldric"},
		"knowledge_graph":
		{
			"nodes":
			{
			...
			"edges":
			[{"from": "aldric", "to": "the_swamp", "relation": "associated_with", "weight": 1.0}]
		}
	}
	...
	var react_outputs = [
		...
		{"thought": "Ready to narrate.", "final": true}
	]
	...
	var findings = await controller._run_director_react_loop(
		"I leave the swamp and head north", "aldric"
	)
	...
	var final_prompt = await pb.build_world_builder_prompt(
		"I leave the swamp and head north", "aldric", findings
	)
...
func test_onboarding_flow() -> bool:
	...
	while true:
		...
		if flow._generated_starters.size() != 3:
			print(
				"[FAIL] Hook generation failure should populate exactly 3 fallback starters. Size: ",
				flow._generated_starters.size()
			)
			remove_child(flow)
			flow.free()
			passed = false
