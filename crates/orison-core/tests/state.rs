...
use orison_core::state::{
    Campaign, CampaignStore, CharacterState, EdgeRow, EmotionEvent, HistoryEntry, HistoryRole,
    InventoryItem, NodeRow, StateError, CURRENT_SCHEMA_VERSION,
};
...
fn migrations_apply_to_a_database_written_by_an_earlier_run() {
    // The migration *mechanism*, exercised end to end on disk: create a
    // database, close it, reopen it through the same migration path, and
    // confirm the version is unchanged and the data survived.
    //
    // With one shipped migration this is version 1 to version 1. §3.6 adds
    // the second migration and turns this into a genuine forward upgrade;
    // the point of having it now is that the mechanism is wired and covered
    // rather than assumed.
    ...
    assert_eq!(
        reopened.load_campaign("saltmarsh").unwrap().unwrap().title,
        "The Salt Tithe"
    );
    assert_eq!(
        reopened.plot_flag("saltmarsh", "tithe_abolished").unwrap(),
        Some("false".into())
    );
}
