...
- [ ] p95 turn latency is no worse than the baseline. **Run, and not met.** `minimal` clears it (p95 15.1 s against 21.2 s); `messy` misses by half again (p95 30.9 s against 20.2 s), and that is the open regression. Phase 4's 63.2 s / 52.9 s improved by 5.0x and 2.1x. The run also read **0% cache reuse** on every turn, which turned out to be the metric rather than the cache: `prompt_eval_count` reports the whole prompt cached or not (`tests/prefix_cache.rs`: a byte-identical 2232-token prefix reported 2232 three times while the work behind it fell from 9858 ms to 179 ms). Reuse is now derived from `prompt_eval_time`, and a re-run on the rebuilt metric is what says whether B-17 reaches a live turn. See [eval_baseline.md](eval_baseline.md#turn-latency--the-live-re-run-phase-56).
